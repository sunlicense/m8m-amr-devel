...
void laserRCallBack(const sensor_msgs::LaserScan::ConstPtr& lscan) {	
	ros::NodeHandle nx;	
	double ld,degperindx,rr,an,radperindx,dm;
	int nleg;
...
void laserLCallBack(const sensor_msgs::LaserScan::ConstPtr& lscan) {	
	...
	if (!lflag) {
		lflag = true;
		ROS_INFO("--- Trolley Left Laser : size=%d. cidx=%d. degidx=%.3f. radidx=%.3f. lidx=%d. dm=%.3f --------",laserL_size,cidx,degperindx,radperindx,lidx,dm);
	}

...
MovNode::MovNode():
  ...
  currentfloor(1),ACTIVATEJACK(false),AGVCARRYTROLLEY(false),OutsideLift(false),
  jackworking(false),jackComm(true)
{
	ros::NodeHandle nh;
...
{
	//if (!jackworking) {
	//	ROS_INFO("--------- MovNode : Jack COmm is DOWN. -----------");
	//	return;
	//}
	if (updown) {
	...
	switch (mstate) {
		...
		case 164:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (agvjackUp) {
					...
					mstate = 1640; //1690; //165 if need to be near wall			
				}
				clearmap_start_time = ros::Time::now();
			}
			break;
		...
		case 1690:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (agvjackUp) {
					mstate = 1691;			
					ROS_INFO("---------- MovNode 1690 : Jack is Up. -------------");
					AGVCARRYTROLLEY = true;
				}
				clearmap_start_time = ros::Time::now();
			}
		...
		case 1691:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				...
				//if (destPoint == 2) {
					...
					mstate = 0;
					ROS_INFO("----------- MovNode 1691 : PickUp Completed. -----------");
				}
				ntucjobstatus = 3; // not ready
				publish_ntuc_status(1,1,targetLP,(int)(avgenergy),ntucjobstatus);
			}
				...
				if (movedone == 0) {
					if (moveflag == 1) {
						...
						if (redirect) {
							mstate = nstate;
							redirect = false;
							ROS_INFO("-------------- MovNode 186 : mstate nstate. ------------------");
						} else {
							mstate = 0;
							ROS_INFO("-------------- MovNode 186 : mstate 0. ------------------");
						}
						if (low_power) {
							ntucjobstatus = 2; 
							publish_ntuc_status(1,5,otargetLP,(int)(avgenergy),ntucjobstatus);
		...
		case 2001: // LIFTAREA_TO_LOBBYAREA
			...
			break;
		case 2002: // LIFTAREA_TO_LOBBYAREA : Call for Lift.
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				...
				mstate = 2003;
				//publish_status("---------- LF->LB : Asking for Lift. ----------");
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 2003: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				if ((ntucstatus == 10) && (liftDoorOpen)) {
					...
					//publish_status("---------- LF->LB : Lift Arrived. Lift Door Opened. ----------");
					mstate = 2004;
				}
				clearmap_start_time = ros::Time::now();	
			}
		...
		case 2005: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				ROS_INFO("-------------MovNode 2005 : AGV Move into lift ----------------");								
				if (dbRequestS("LFIN")) {
					movedone = 0;
					moveflag = 1;			
					redirect = true;
					...
					//publish_status("---------- LF->LB : Moving into Lift. ----------");
				} else {
					ROS_INFO("---------MovNode 2005 : Inside Lift LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 2006: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
			...
			break;
		case 2007: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (!agvjackUp) {
					// AGV rotate 180.0
					movedone = 1;
			...
			break;
		case 2008: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						...
						liftDoorOpen = false;
						publish_sound(ASKLIFT,0,0);
						publish_lift(LIFTGOTO,currentfloor,1,1,0);  // ask for lift to ground floor
						publish_status("---------- LF->LB : Asking for Lift to go to ground floor. ----------");
						mstate = 2009;
					}
				}
				clearmap_start_time = ros::Time::now();	
			...
			break;	
		case 2009: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				if (agvjackUp) {
			...
			break;
		case 2020:  // LIFTAREA_TO_LOBBYAREA : no trolley inside lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				// AGV turn around inside lift
				publish_lift(AGVINLIFT,currentfloor,0,1,0);  // robot in lift
				mstate = 2021;	
			...
			break;
		case 2021: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (!agvjackUp) {
					// AGV rotate 180.0
					movedone = 1;
			...
			break;
		case 2022: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						publish_sound(ASKLIFT,0,0);
			...
			break;	
		case 2023: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				if ((ntucstatus == 11) && (liftDoorOpen)) {
					ntucstatus = 0;
					liftDoorOpen = false;
			...
			break;		
		case 2010: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				rn.getParam("ProfileMoveObsDist",ProfileMoveObsDist);
				if (ProfileMoveObsDist > DistClearance) {
					mstate = 2011;
			...
			break;
		case 2011: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				...
				if (dbRequestS("LFLBD")) {
					...
					ROS_INFO("--------- MovNode 2011 : LFLBD  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;		
		case 2012: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
			...
			break;
		case 2013: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {				
				if (ntucLobbyDoor == 1) {
					mstate = 2014;
					ntucDoorOpenRequestCnt = 0;
			...
			break;
		case 2014: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {				
				if (ntucLobbyDoor == 2) {
					mstate = 2015;
					ntucDoorOpenRequestCnt = 0;
					...
					ROS_INFO("-------------MovNode 2014 : Lobby Door Command Activated. ----------------");
					//publish_status("---------- LF->LB : Lobby Door Opened. ----------");
				} else {
					ntucDoorOpenRequestCnt++;
			...
			break;
		case 2015: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				rn.getParam("ProfileMoveObsDist",ProfileMoveObsDist);
				if (ProfileMoveObsDist > DistClearance) {
					mstate = 2016;
			...
			break;
		case 2016: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
			...
			break;
		case 2017: // LIFTAREA_TO_LOBBYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {	
			...
			break;
		case 2018:  // go to docking
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (dbRequestS("LBLB")) {
					//publish_status("---------- LF->LB : No new Job. Going back to Docking. ----------");
					mstate = 3500; // premove
					nstate = 2019;
				} else {
					ROS_INFO("---------MovNode 2018 :  LobbyARea LoadingBay LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		case 2024:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				ROS_INFO("---------MovNode 2024 : Checking loadingBay Status  -----------");
				rn.getParam("LoadingBayStatus",LoadingBayStatus);
				if (!LoadingBayStatus) {
					mstate = 2025;
					ROS_INFO("---------MovNode 2024 : loadingBay Clear  -----------");
				}
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 2025:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (dbRequestS("LURET")) {
					...
					ROS_INFO("---------MovNode 2025 :  LURET LP  Error. -----------");
				}
			}
			break;
		case 2026:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (dbRequestS("REF")) { // ***DOCK***
					...
					ROS_INFO("---------MovNode 2026 :  DOCK LP  Error. -----------");
				}	
				clearmap_start_time = ros::Time::now();
			}
			break;
		...
		case 2051: // LIFTAREA_TO_LAUNDRYAREA
			...
			break;
		case 2052: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				publish_sound(ASKLIFT,0,0);
			...
			break;
		case 2053: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if ((ntucstatus == 10) && (liftDoorOpen)) {
					// robot move into lift
					ntucstatus = 0;
		...
		case 2055: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				ROS_INFO("-------------MovNode 2055 : AGV Move into lift ----------------");								
				if (dbRequestS("LFIN")) {
					movedone = 0;
					moveflag = 1;
					redirect = true;
		...
		case 2058: // LIFTAREA_TO_LAUNDRYAREA
			...
			break;
		case 2059: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (agvjackUp) {
					if ((ntucstatus == 11) && (liftDoorOpen)) {
						ntucstatus = 0;
			...
			break;	
		case 2072: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				// AGV turn around inside lift
				publish_lift(AGVINLIFT,currentfloor,0,1,0);  // robot in lift
				mstate = 2073;		
			...
			break;
		case 2073: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (!agvjackUp) {
					// AGV rotate 180.0
					movedone = 1;
			...
			break;
		case 2074: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						publish_sound(ASKLIFT,0,0);
			...
			break;	
		case 2075: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				if ((ntucstatus == 11) && (liftDoorOpen)) {
					ntucstatus = 0;
					liftDoorOpen = false;
			...
			break;	
		case 2060: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				rn.getParam("ProfileMoveObsDist",ProfileMoveObsDist);
				if (ProfileMoveObsDist > DistClearance) {
					mstate = 2061;
			...
			break;
		case 2061: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				publish_sound(MOVEOUTOFLIFT,0,0);			
				if (dbRequestS("LFLBD")) {
					movedone = 0;
			...
			break;	
		case 2062: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
			...
			break;
		case 2063: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {				
				if (ntucLobbyDoor == 1) {
					mstate = 2064;
					ntucDoorOpenRequestCnt = 0;
			...
			break;
		case 2064: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {				
				if (ntucLobbyDoor == 2) {
					mstate = 2065;
					ntucDoorOpenRequestCnt = 0;
			...
			break;
		case 2065: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				rn.getParam("ProfileMoveObsDist",ProfileMoveObsDist);
				if (ProfileMoveObsDist > DistClearance) {
					mstate = 2066;
			...
			break;
		case 2066: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				if (dbRequestS("LBLB")) {
					movedone = 0;
					moveflag = 1;
					...
					ROS_INFO("--------- MovNode 2066 : LBLB LP  Error. -----------");
				}			
			}
			break;
		case 2067: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				rn.getParam("LoadingBayStatus",LoadingBayStatus);
				if (!LoadingBayStatus) {
					mstate = 2068;
			...
			break;
		case 2068: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (dbRequestS("LURET")) {
					movedone = 0;
					moveflag = 1;
					...
					ROS_INFO("---------MovNode 2068 :  LURET LP  Error. -----------");
				}
			}
			break;
		case 2069: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				restoreLP();   // restore final LP				
				movedone = 0;				
				moveflag = 1;
			...
			break;
		case 2070: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
		...
		case 2071:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (dbRequestS("REF")) {  // *** DOCK ***
					...
					ROS_INFO("---------MovNode 2069 :  DOCK LP  Error. -----------");
				}	
				clearmap_start_time = ros::Time::now();
			}
			break;
		...
		case 2104: // LOBBYAREA_TO_LAUNDRYAREA
			...
			break;
		case 2105: // LOBBYAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {	
				mstate = 3600;  // check if job in queue
				nstate = 2106;
				clearmap_start_time = ros::Time::now();	
		...
		case 2153: // LOBBYAREA_TO_LIFTAREA
			...
			break;
		case 2154: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {				
				if (ntucLobbyDoor == 2) {
					mstate = 2155;
					ntucDoorOpenRequestCnt = 0;
		...
		case 2155: // LOBBYAREA_TO_LIFTAREA
			...
			break;
		case 2156: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				if (dbRequestS("LFLD")) {
					movedone = 0;					
					moveflag = 1;
					...
					ROS_INFO("---------MovNode 2156 : LFLD LP Error -----------");
				} 
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 2157: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				liftDoorOpen = false;
				publish_sound(ASKLIFT,0,0);
				ROS_INFO("-------------MovNode 2157 : AGV Requesting for Lift  ----------------");
				publish_lift(ASKLIFTTOCOME,1,0,1,0);
				mstate = 2158;	
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 2158: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if ((ntucstatus == 10) && (liftDoorOpen)) {
					// robot move into lift
					ntucstatus = 0;
			...
			break;
		case 2159: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				rn.getParam("ProfileMoveObsDist",ProfileMoveObsDist);
				if (ProfileMoveObsDist > DistClearance) {
					mstate = 2160;
			...
			break;
		case 2160: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				ROS_INFO("-------------MovNode 2160 : AGV Move into lift ----------------");								
				if (dbRequestS("LFIN")) {
					movedone = 0;					
					moveflag = 1;
					redirect = true;
					...
					ROS_INFO("---------MovNode 2160 : Inside Lift LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 2161: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				// AGV turn around inside lift
			...
			break;
		case 2162: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (!agvjackUp) {
					// AGV rotate 180.0
					movedone = 1;
			...
			break;
		case 2163: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						restoreLP();
			...
			break;		
		case 2164: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (agvjackUp) {
					if ((ntucstatus == 11) && (liftDoorOpen)) {
						ntucstatus = 0;
			...
			break;
		case 2165:  //// LOBBYAREA_TO_LIFTAREA : no trolley on agv
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				// AGV turn around inside lift
				publish_lift(AGVINLIFT,currentfloor,0,1,0);  // robot in lift
				mstate = 2166;	
			...
			break;
		case 2166: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (!agvjackUp) {
					// AGV rotate 180.0
					movedone = 1;
			...
			break;
		case 2167: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						restoreLP();
			...
			break;	
		case 2168: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				if ((ntucstatus == 11) && (liftDoorOpen)) {
					ntucstatus = 0;
					liftDoorOpen = false;
			...
			break;
		case 2169: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				rn.getParam("ProfileMoveObsDist",ProfileMoveObsDist);
				if (ProfileMoveObsDist > DistClearance) {
					mstate = 2170;
			...
			break;
		case 2170: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				publish_sound(MOVEOUTOFLIFT,0,0);			
				OutsideLift = true;
			...
			break;
		case 2171: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {	
				//publish_lift(AGVOUTSIDE,currentfloor,0,0,0);
				//publish_lift(KITCHEN,currentfloor,0,0,1);
				mstate = 3600;
				nstate = 2172;		// reached liftarea ward. check if job in queue
				clearmap_start_time = ros::Time::now();	
			}
			break;		
		case 2172: //**** return to DOCK
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				if (!agvjackUp) {
					if (dbRequestS("LFLD")) {
						mstate = 3500; // premove
						...
						ROS_INFO("---------MovNode 2172 :  LFLD LP  Error. -----------");
					}
				}
				clearmap_start_time = ros::Time::now();	
			}
		...
		case 2174:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				publish_sound(ASKLIFT,0,0);
				liftDoorOpen = false;
				ROS_INFO("-------------MovNode 2174 : AGV Requesting for Lift  ----------------");
				publish_lift(ASKLIFTTOCOME,currentfloor,1,1,0);
				mstate = 2175;	
				clearmap_start_time = ros::Time::now();
			}
			break;
		...
		case 2177:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				ROS_INFO("-------------MovNode 2177 : AGV Move into lift ----------------");								
				if (dbRequestS("LFIN")) {
					movedone = 0;					
					moveflag = 1;
					redirect = true;
					...
					ROS_INFO("---------MovNode 2177 : Inside Lift LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						...
						clearmap_start_time = ros::Time::now();		
						publish_sound(ASKLIFT,0,0);
						publish_lift(LIFTGOTO,currentfloor,1,1,0);  
						mstate = 2181;
					}
				}
				clearmap_start_time = ros::Time::now();	
		...
		case 2187://
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				...
				if (dbRequestS("LFLBD")) {
					...
					ROS_INFO("--------- MovNode 2187 : LFLBD  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		case 2189:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {				
				if (ntucLobbyDoor == 1) {
					mstate = 2190;
					ntucDoorOpenRequestCnt = 0;
					ROS_INFO("-------------MovNode 2189 : Lobby Door Command Received. ----------------");
				} else {
					publish_fButton(301);
					ntucDoorOpenRequestCnt++;
					if (ntucDoorOpenRequestCnt > 20) {
		...
		case 2190:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {				
				if (ntucLobbyDoor == 2) {
					...
					ntucLobbyDoor = 0;
					publish_sound(LOBBYDOOROPENED,0,0);
					ROS_INFO("------------- MovNode 2190 : Lobby Door Command Activated. ----------------");
				} else {
					ntucDoorOpenRequestCnt++;
					if (ntucDoorOpenRequestCnt > 25) {
						publish_sound(LOBBYDOORCONTROLERROR,0,0); //Lobby Door Error. Door Controller Error
		...
		case 2192:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				if (dbRequestS("LBLB")) {
					...
					ROS_INFO("--------- MovNode 2192 : LoadingBay_lobby LP  Error. -----------");
				}			
			}
			break;
		case 2193: 
		...
		case 2194:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (dbRequestS("LURET")) {
					...
					ROS_INFO("---------MovNode 2194 :  LURET LP  Error. -----------");
				}
			}
			break;
		case 2195:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (dbRequestS("REF")) {  // *** DOCK ***
					...
					ROS_INFO("---------MovNode 2195 : Dock LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		case 2204: // LAUNDRYAREA_TO_LIFTAREA : Lobby
			...
			break;
		case 2205: // LAUNDRYAREA_TO_LIFTAREA : Lobby
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				publish_fButton(301);// request to open lobby door						
				publish_sound(REQUESTLOBYDOOROPEN,0,0);
				ROS_INFO("-------------MovNode 2205 : Request to open Lobby Door. ----------------");							
				ntucDoorOpenRequestCnt = 0;
				mstate = 2206;
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 2206: // LAUNDRYAREA_TO_LIFTAREA : Lobby
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {				
				if (ntucLobbyDoor == 1) {
					mstate = 2207;
					ntucDoorOpenRequestCnt = 0;
					ROS_INFO("-------------MovNode 2206 : : Lobby Door Command Received. ----------------");
				} else {
					publish_fButton(301);
					ROS_INFO("-------------MovNode 2206 : Request to open Lobby Door Again. ----------------");
					ntucDoorOpenRequestCnt++;
					if (ntucDoorOpenRequestCnt > 20) {
						publish_sound(LOBBYDOORCMDERROR,0,0); //Lobby Door Error. Command not recevied
						ntucDoorOpenRequestCnt = 0;
			...
			break;
		case 2207: // LAUNDRYAREA_TO_LIFTAREA : Lobby
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {				
				if (ntucLobbyDoor == 2) {
					mstate = 2208;
					ntucDoorOpenRequestCnt = 0;
					ntucLobbyDoor = 0;
					publish_sound(LOBBYDOOROPENED,0,0);
					ROS_INFO("-------------MovNode 2207 : Lobby Door Command Activated. ----------------");
				} else {
					ntucDoorOpenRequestCnt++;
					if (ntucDoorOpenRequestCnt > 25) {
						publish_sound(LOBBYDOORCONTROLERROR,0,0); //Lobby Door Error. Door Controller Error
			...
			break;
		case 2208: // LAUNDRYAREA_TO_LIFTAREA : Lobby
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				rn.getParam("ProfileMoveObsDist",ProfileMoveObsDist);
				if (ProfileMoveObsDist > DistClearance) {
					mstate = 2209;
			...
			break;
		case 2209: // LAUNDRYAREA_TO_LIFTAREA : Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				if (dbRequestS("LFLD")) {
					movedone = 0;			
					moveflag = 1;
					...
					mstate = 310; // 310
					nstate = 2210;
					ROS_INFO("---------MovNode 2209 : Go to LFLD -----------");					
				} else {
					ROS_INFO("---------MovNode 2209 : Outside Lift LP Error -----------");
				} 
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 2210: // LAUNDRYAREA_TO_LIFTAREA : Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				publish_sound(ASKLIFT,0,0);
				ROS_INFO("------------- MovNode 2210 :  AGV Requesting for Lift  ----------------");
				publish_lift(ASKLIFTTOCOME,1,0,1,0);
				mstate = 2211;
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 2211: // LAUNDRYAREA_TO_LIFTAREA : Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				if ((ntucstatus == 10) && (liftDoorOpen)) {
					// robot move into lift
					ntucstatus = 0;
			...
			break;
		case 2212: // LAUNDRYAREA_TO_LIFTAREA : Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				rn.getParam("ProfileMoveObsDist",ProfileMoveObsDist);
				if (ProfileMoveObsDist > DistClearance) {
					mstate = 2213;
			...
			break;
		case 2213: // LAUNDRYAREA_TO_LIFTAREA : Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				ROS_INFO("-------------MovNode 2213 : AGV Move into lift ----------------");								
				if (dbRequestS("LFIN")) {
					movedone = 0;			
					moveflag = 1;				
					redirect = true;
					...
					ROS_INFO("---------MovNode 2213 : Inside Lift LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 2214: // LAUNDRYAREA_TO_LIFTAREA : Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				publish_lift(AGVINLIFT,1,0,1,0);  // robot in lift
				liftMechanism(false); // lower jack
				ROS_INFO("---------- MovNode 2214 : Lower Jack. ----------");						
				mstate = 2215;	
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 2215: // LAUNDRYAREA_TO_LIFTAREA : Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (!agvjackUp) {
					// AGV rotate 180.0
					ROS_INFO("---------- MovNode 2215 : Turn around inside lift. ----------");
					movedone = 1;
					moveflag = 0;
					pos.linear.z = 42.0; // 
					pos.angular.z = 180.0;
			...
			break;
		case 2216: // LAUNDRYAREA_TO_LIFTAREA : Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						liftMechanism(true); // raise jack
						ROS_INFO("---------- MovNode 2216 : Raise Jack. ----------");
						mstate = 2217;
					}
				}
				clearmap_start_time = ros::Time::now();	
			...
			break;
		case 2217: // LAUNDRYAREA_TO_LIFTAREA : Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (agvjackUp) {
					ROS_INFO("---------- MovNode 2217 : Ask for lift. ----------");
					restoreLP();						
					publish_sound(ASKLIFT,0,0);
					publish_lift(LIFTGOTO,currentfloor,checkFloor(targetLP),1,0);  
					mstate = 2218;
					liftDoorOpen = false;
				}
				clearmap_start_time = ros::Time::now();	
			...
			break;
		case 2218: // LAUNDRYAREA_TO_LIFTAREA : Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.0)) ) {
				if ((ntucstatus == 11) && (liftDoorOpen)) {
					liftDoorOpen = false;
					ntucstatus = 0;
			...
			break;
		case 2219:  // no trolley on agv // LAUNDRYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				publish_lift(AGVINLIFT,currentfloor,0,1,0);  // robot in lift
				mstate = 2220;		
				clearmap_start_time = ros::Time::now();
			...
			break;
		case 2220: // LAUNDRYAREA_TO_LIFTAREA : Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (!agvjackUp) {
					// AGV rotate 180.0
					movedone = 1;
			...
			break;
		case 2221: // LAUNDRYAREA_TO_LIFTAREA : Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						restoreLP();
			...
			break;	
		case 2222: // LAUNDRYAREA_TO_LIFTAREA : Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				if ((ntucstatus == 11) && (liftDoorOpen)) {
					ntucstatus = 0;
					liftDoorOpen = false;
			...
			break;	
		case 2223: // LAUNDRYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				rn.getParam("ProfileMoveObsDist",ProfileMoveObsDist);
				if (ProfileMoveObsDist > DistClearance) {					
					publish_sound(MOVEOUTOFLIFT,0,0);		
					...
					liftcfloor = currentfloor;
					liftdfloor = 0;
					ROS_INFO("---------- MovNode 2223 : Move out of Lift to Ward  ----------");								
					movedone = 0;			
					moveflag = 1;
					redirect = true;
					mstate = 310; // 	310	
			...
			break;
		case 2224: // LAUNDRYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {	
				//publish_lift(AGVOUTSIDE,0,currentfloor,0,0);
				//publish_lift(KITCHEN,0,currentfloor,0,1);
				ROS_INFO("---------- MovNode 2224 : AGV outside of Lift. ----------");
				mstate = 3600;  // check if new job in queue
				nstate = 2225;
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 2225: //**** return to DOCK
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (!agvjackUp) {
					if (dbRequestS("LFLD")) {
						ROS_INFO("---------- MovNode 2225 : Move to LFLD. --------");
						mstate = 3500; // premove
						nstate = 2226;
					} else {
						ROS_INFO("---------MovNode 2225 :  LFLD LP  Error. -----------");
					}
				}
				clearmap_start_time = ros::Time::now();	
			}
		...
		case 2227:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				publish_sound(ASKLIFT,0,0);
				ROS_INFO("-------------MovNode 2227 : AGV Requesting for Lift  ----------------");
				publish_lift(ASKLIFTTOCOME,currentfloor,1,1,0);
				mstate = 2228;
				clearmap_start_time = ros::Time::now();
			}
		...
		case 2230:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				ROS_INFO("-------------MovNode 2230 : AGV Move into lift ----------------");								
				if (dbRequestS("LFIN")) {
					movedone = 0;			
					moveflag = 1;				
					redirect = true;
					...
					ROS_INFO("---------MovNode 2230 : Inside Lift LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 2231:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				publish_lift(AGVINLIFT,currentfloor,0,1,0);  // robot in lift
				liftMechanism(false); // lower jack
				ROS_INFO("---------- MovNode 2231 : Lower Jack. ----------");						
				mstate = 2232;
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 2232:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (!agvjackUp) {
					// AGV rotate 180.0
					ROS_INFO("---------- MovNode 2232 : Turn around inside lift. ----------");
					movedone = 1;
					moveflag = 0;
					pos.linear.z = 42.0; // 
					pos.angular.z = 180.0;
		...
		case 2233:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						liftMechanism(true); // raise jack
						ROS_INFO("---------- MovNode 2233 : Raise Jack. ----------");
						mstate = 2234;
					}
				}
				clearmap_start_time = ros::Time::now();	
		...
		case 2234:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (agvjackUp) {
					ROS_INFO("---------- MovNode 2234 : Ask for lift. ----------");
					//clearmap_start_time = ros::Time::now();		
					publish_sound(ASKLIFT,0,0);
					publish_lift(LIFTGOTO,currentfloor,1,1,0);  
					mstate = 2235;
		...
		case 2241://
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				...
				if (dbRequestS("LFLBD")) {
					...
					ROS_INFO("--------- MovNode 2241 : LFLBD  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 2242: 
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				publish_lift(AGVOUTSIDE,0,1,0,0);
				publish_fButton(301);
				publish_sound(REQUESTLOBYDOOROPEN,0,0);							
				ntucDoorOpenRequestCnt = 0;
				mstate = 2243;
		...
		case 2243:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {				
				if (ntucLobbyDoor == 1) {
					mstate = 2244;
					ntucDoorOpenRequestCnt = 0;
					ROS_INFO("-------------MovNode 2243 : Lobby Door Command Received. ----------------");
				} else {
					publish_fButton(301);
					ntucDoorOpenRequestCnt++;
					if (ntucDoorOpenRequestCnt > 20) {
		...
		case 2244:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {				
				if (ntucLobbyDoor == 2) {
					...
					ntucLobbyDoor = 0;
					publish_sound(LOBBYDOOROPENED,0,0);
					ROS_INFO("------------- MovNode 2244 : Lobby Door Command Activated. ----------------");
				} else {
					ntucDoorOpenRequestCnt++;
					if (ntucDoorOpenRequestCnt > 25) {
						publish_sound(LOBBYDOORCONTROLERROR,0,0); //Lobby Door Error. Door Controller Error
		...
		case 2246:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				if (dbRequestS("LBLB")) {
					...
					ROS_INFO("--------- MovNode 2246 : LoadingBay_lobby LP  Error. -----------");
				}			
			}
			break;
		case 2247: 
		...
		case 2248:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (dbRequestS("LURET")) {
					...
					ROS_INFO("---------MovNode 2248 :  LURET LP  Error. -----------");
				}
			}
			break;
		case 2249:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				if (dbRequestS("REF")) {  // *** DOCK ***
					...
					ROS_INFO("---------MovNode 2249 : Dock LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		case 3006:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.0)) ) {
				if (dbRequestS("LBLB")) {
					...
					ROS_INFO("--------- MovNode 3006 : LBLB LP  Error. -----------");
				}		
			}
			break;
		case 3007: 
		...
		case 3008:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				ROS_INFO("---------MovNode 3008 : Checking loadingBay Status  -----------");
				rn.getParam("LoadingBayStatus",LoadingBayStatus);
				if (!LoadingBayStatus) {
					mstate =3009;
					ROS_INFO("---------MovNode 3008 : loadingBay Clear  -----------");
				}	
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 3009:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (dbRequestS("LURET")) {
					...
					ROS_INFO("---------MovNode 3009 :  LURET LP  Error. -----------");
				}
			}
			break;
		case 3010:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (dbRequestS("REF")) {  // *** DOCK ***
					...
					ROS_INFO("---------MovNode 3010 : Dock LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		case 3052: // LIFTAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				publish_sound(ASKLIFT,0,0);
				liftDoorOpen = false;
				ROS_INFO("-------------MovNode 3052 : AGV Requesting for Lift  ----------------");
				publish_lift(ASKLIFTTOCOME,currentfloor,0,1,0);
				mstate = 3053;
				clearmap_start_time = ros::Time::now();
			}
		...
		case 3053: // LIFTAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if ((ntucstatus == 10) && (liftDoorOpen)) {
					...
					liftDoorOpen = false;
					mstate = 3054;
					//ROS_INFO("------------- MovNode 3053 : AGV Move into lift ----------------");					
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 3054: // LIFTAREA_TO_LIFTAREA
			...
			break;
		case 3055: // LIFTAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				ROS_INFO("-------------MovNode 3055 : AGV Move into lift ----------------");								
				if (dbRequestS("LFIN")) {
					movedone = 0;			
					moveflag = 1;						
					redirect = true;
					...
					ROS_INFO("---------MovNode 3055 : Inside Lift LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 3056: // LIFTAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				publish_lift(AGVINLIFT,currentfloor,0,1,0);  // robot in lift
				liftMechanism(false); // lower jack
				ROS_INFO("---------- MovNode 3056 : Lower Jack. ----------");						
				mstate = 3057;	
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 3057: // LIFTAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (!agvjackUp) {
					// AGV rotate 180.0
					ROS_INFO("---------- MovNode 3057 : Turn around inside lift. ----------");
					movedone = 1;
					moveflag = 0;
					pos.linear.z = 42.0; // 
					pos.angular.z = 180.0;
			...
			break;
		case 3058: // LIFTAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						liftMechanism(true); // raise jack
						ROS_INFO("---------- MovNode 3058 : Raise Jack. ----------");
						mstate = 3059;
					}
				}
				clearmap_start_time = ros::Time::now();	
			...
			break;
		case 3059: // LIFTAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (agvjackUp) {
					ROS_INFO("---------- MovNode 3059 : Ask for lift. ----------");				
					restoreLP();	
					publish_sound(ASKLIFT,0,0);
					publish_lift(LIFTGOTO,currentfloor,checkFloor(targetLP),1,0);  
					mstate = 3060;
			...
			break;
		case 3060: // LIFTAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.0)) ) {
				if ((ntucstatus == 11) && (liftDoorOpen)) {
					liftDoorOpen = false;
					ntucstatus = 0;
			...
			break;
		case 3061:  // no trolley on agv // LIFTAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				// AGV turn around inside lift
				publish_lift(AGVINLIFT,currentfloor,0,1,0);  // robot in lift
				mstate = 3062;		
			...
			break;
		case 3062: // LIFTAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (!agvjackUp) {
					// AGV rotate 180.0
					movedone = 1;
			...
			break;
		case 3063: // LIFTAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						restoreLP();
			...
			break;	
		case 3064: // LIFTAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				if ((ntucstatus == 11) && (liftDoorOpen)) {
					ntucstatus = 0;
					liftDoorOpen = false;
		...
		case 3065: // LIFTAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				rn.getParam("ProfileMoveObsDist",ProfileMoveObsDist);
				if (ProfileMoveObsDist > DistClearance) {					
					publish_sound(MOVEOUTOFLIFT,0,0);		
					...
					liftcfloor = currentfloor;
					liftdfloor = 0;
					ROS_INFO("---------- MovNode 3065 : Move out of Lift to Ward . ----------");		
					movedone = 0;			
					moveflag = 1;	
					redirect = true;
					mstate = 310; // 	310	
			...
			break;	
		case 3066: // LIFTAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {	
				//publish_lift(AGVOUTSIDE,currentfloor,0,0,0);
				//publish_lift(KITCHEN,currentfloor,0,0,1);
				ROS_INFO("---------- MovNode 3066 : AGV outside of Lift. ----------");
				mstate = 3600;  //check if job in queue
				nstate = 3067;
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 3067:  // return to dock. 
			ROS_INFO("----------- MovNode 3067 : RETURN TO DOCK. LIFTAREA_TO_DOCK ---------------");
			mstate = 3500; // pre-move
			nstate = 3068;
			break;
		case 3068:
				...
				if (movedone == 0) {
					if (moveflag == 1) {
						if (dbRequestS("LFLD")) {
							...
							ROS_INFO("--------- MovNode 3068 : LFLD Error -----------");
						}
					} 
				} 
				clearmap_start_time = ros::Time::now();
		...
		case 3069:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				publish_sound(ASKLIFT,0,0);
				ROS_INFO("-------------MovNode 3069 : AGV Requesting for Lift  ----------------");
				publish_lift(ASKLIFTTOCOME,currentfloor,0,1,0);
				mstate = 3070;
				clearmap_start_time = ros::Time::now();
			}
		...
		case 3070:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if ((ntucstatus == 10) && (liftDoorOpen)) {
					...
					liftDoorOpen = false;
					mstate = 3071;
					ROS_INFO("------------- MovNode 3070 : AGV Move into lift ----------------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		case 3072:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				ROS_INFO("-------------MovNode 3072 : AGV Move into lift ----------------");								
				if (dbRequestS("LFIN")) {
					movedone = 0;			
					moveflag = 1;						
					redirect = true;
					...
					ROS_INFO("---------MovNode 3072 : Inside Lift LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 3073:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				publish_lift(AGVINLIFT,currentfloor,0,1,0);  // robot in lift
				liftMechanism(false); // lower jack
				ROS_INFO("---------- MovNode 3073 : Lower Jack. ----------");						
				mstate = 3074;	
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 3074:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (!agvjackUp) {
					// AGV rotate 180.0
					ROS_INFO("---------- MovNode 3074 : Turn around inside lift. ----------");
					movedone = 1;
					moveflag = 0;
					pos.linear.z = 42.0; // 
					pos.angular.z = 180.0;
		...
		case 3075:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						liftMechanism(true); // raise jack
						ROS_INFO("---------- MovNode 3075 : Raise Jack. ----------");
						mstate = 3076;
					}
				}
				clearmap_start_time = ros::Time::now();	
		...
		case 3076:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (agvjackUp) {
					ROS_INFO("---------- MovNode 3076 : Ask for lift. ----------");									
					publish_sound(ASKLIFT,0,0);
					publish_lift(LIFTGOTO,currentfloor,1,1,0);  
					mstate = 3080;
					liftDoorOpen = false;
			...
			break;
		case 3077:  // no trolley
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				publish_lift(AGVINLIFT,currentfloor,0,1,0);  // robot in lift
				ROS_INFO("---------- MovNode 3077 : Lower Jack. ----------");						
				mstate = 3078;
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 3078:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (!agvjackUp) {
					// AGV rotate 180.0
					ROS_INFO("---------- MovNode 3078 : Turn around inside lift. ----------");
					movedone = 1;
					moveflag = 0;
					pos.linear.z = 42.0; // 
					pos.angular.z = 180.0;
		...
		case 3079:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						ROS_INFO("---------- MovNode 3079 : Ask for lift. ----------");									
						publish_sound(ASKLIFT,0,0);
						publish_lift(LIFTGOTO,currentfloor,1,1,0);  
						liftDoorOpen = false;
						mstate = 3080;
		...
		case 3081:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				...
				if (ProfileMoveObsDist > DistClearance) {					
					publish_sound(MOVEOUTOFLIFT,0,0);		
					ROS_INFO("---------- MovNode 3081 : Move out of Lift to lobby Door . ----------");		
					if (dbRequestS("LFLBD")) {
						movedone = 0;
						moveflag = 1;
						redirect = true;
						...
						ROS_INFO("--------- MovNode 3081 : LFLBD  Error. -----------");
					}		
				}
				clearmap_start_time = ros::Time::now();	
			}
		...
		case 3083:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {				
				if (ntucLobbyDoor == 1) {
					mstate = 3084;
					ntucDoorOpenRequestCnt = 0;
					ROS_INFO("-------------MovNode 3083 : Lobby Door Command Received. ----------------");
				} else {
					publish_fButton(301);
					ntucDoorOpenRequestCnt++;
					if (ntucDoorOpenRequestCnt > 20) {
		...
		case 3084:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {				
				if (ntucLobbyDoor == 2) {
					...
					ntucLobbyDoor = 0;
					publish_sound(LOBBYDOOROPENED,0,0);
					ROS_INFO("------------- MovNode 3084 : Lobby Door Command Activated. ----------------");
				} else {
					ntucDoorOpenRequestCnt++;
					if (ntucDoorOpenRequestCnt > 25) {
						publish_sound(LOBBYDOORCONTROLERROR,0,0); //Lobby Door Error. Door Controller Error
		...
		case 3086:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				if (dbRequestS("LBLB")) {
					...
					ROS_INFO("--------- MovNode 3086 : LoadingBay_lobby LP  Error. -----------");
				}			
			}
			break;
		case 3087: 
		...
		case 3088:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (dbRequestS("LURET")) {
					...
					ROS_INFO("---------MovNode 3088 :  LURET LP  Error. -----------");
				}
			}
			break;
		case 3089:
		...
		case 3600: // check if new job in queue
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				...
				if ((numLPInQ > 0) && botrun) {
					...
					mstate = 3601;
					ROS_INFO("------------ MovNode 3600 : No Job in queue. Wait 5 secs ----------");					
				}
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 3601:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(3.5)) ) {
				rn.getParam("numLPInQ",numLPInQ);
				if ((numLPInQ > 0) && botrun) {
					mstate = 0;
					...
					mstate = nstate;
					ROS_INFO("---------- MovNode 3601 : No Job in queue. Continue with nstate --------");					
				}
				clearmap_start_time = ros::Time::now();
			}
			break;
...
int main(int argc, char **argv)
{
  ...
  while (true) {  
		...
		if (c4 == 10) {
			//c4 = 0;
			c5 = 0;
			mNode.jackworking = false;
			mNode.publish_fButton(601);
		}
		c5++;
		if (c5 == 30) {
			c4 = 0;
			if (!mNode.jackworking) {
				mNode.jackComm = false;
				ROS_INFO("--------- MovNode : Jack Comm is Down ----------");
			} else {
				mNode.jackComm = true;
			}

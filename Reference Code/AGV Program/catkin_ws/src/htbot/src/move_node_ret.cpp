...
*		Date Modified : 
		...
		19.7.21
			1. 12.10pm : adjusted the steps to pick up trolley
		21.7.21
			1. 9.30am : added NTUCDOCK to switch from REF to DOCK when docking station ready
		27.7.21 
			1. 3pm : added different drop point LP.
		28.7.21
			1. 9.45am : added stopping ntuc navigation halfway.
		30.7.21
			1. 12.54pm : add moving to goal and turn to orientation
		2.8.21 
			1. 6.30pm : added ptpmovealign for LULB, LBLB1, LBLB2
		5.8.21
			1. 12pm : change the dropping of trolley in the laundry area similiar to dropping at lift area
			2. 5pm : change workflow to use ptp move.
		6.8.21
			1. 10am : change movement flow for laundry to liftarea
			2. 1pm : added calcQuatLP(string fLP, string tLP)
		9.8.21 
			1. 5pm : added test for pathfollowing
			2. 5.30pm : change publish_sound to original
		11.8.21
			1. 11am : updated the movement flow
		12.8.21
			1. 10am : change return to docking with first point of checking for job in front of the
		31.8.21 
			1. 12pm : set loadingbaystatus just before checking of loadingbay is clear to move
*		8.9.21 
			1. 11am : change some sound to combined sound.
*     
*/
...
#define TROLLEY1RETREAT 1.2
#define TROLLEY2RETREAT 1.25
#define TROLLEY3RETREAT 1.3
...
public:
	...
	bool dbRequestT(std::string lps);
	...
	void calcQuatLP(string fLP, string tLP);
	...
	double findtrolleyretreat(int type);
	int checkFloor(int lp);
	void GoalLPpub(double x,double y, double rz, double rw);
	void initialpose_LP(double x, double y);
	...
	bool Change_Nav,normalPath,NTUCDOCK;
...
MovNode::MovNode():
  ...
  jackworking(false),jackComm(true),rstate(0),LaundryDoorGapOut(1.05),LaundryDoorGapIn(0.9),jackUpCmdRev(false),
	...
	agvjackstatus(true),NTUCDOCK(false),ZeroLocalisation(false)
{
	ros::NodeHandle nh;
	...
	goalLP_pub = nh.advertise<htbot::move>("goalLP", 10);
  ...
  // Uno
	...
	nh.getParam("NTUCDOCK",NTUCDOCK);
}

void MovNode::initialpose_LP(double x, double y) 
{
	...
	ipose.pose.pose.position.x = x;
	ipose.pose.pose.position.y = y;
	...
	ROS_INFO("-------MovNode : Initialpose LP . x=%.3f.  y=%.3f. ---------",tx,ty);
}

void MovNode::GoalLPpub(double x,double y, double rz, double rw)
{
	htbot::move cmd;	
	cmd.x = x;
	cmd.y = y;
...
double MovNode::findtrolleyretreat(int type) {
	double len;
	switch(type) {
		case 1:
			len = TROLLEY1RETREAT;
...
void MovNode::calcQuatLP(string fLP, string tLP)
{
	double an,yawp,diff;
	tf2::Quaternion quat;
	geometry_msgs::Quaternion qq;
...
void MovNode::buttonCallback(const std_msgs::UInt16::ConstPtr& msg)
{
	...
	switch (butNo) {
		...
		case 21:  // robot move on
			moveOn = true;
			break;
		case 22: // stop ntuc nav
			mstate = 0;
			ROS_INFO("---------- mNode : Stopping NTUC Navigation. -----------------");
			break;
		case 23: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign LUENT -----------------");
			if (dbRequestS("LUENT")) {
				GoalLPpub(tx,ty,rz,rw);
				pos.linear.z = 42.7; // similiar to post move without scan matching
				pos.angular.z = 1.0; // turn to orientation ? 0.0 = no
				pos_pub.publish(pos);
			}
			break;
		case 24: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign LURET -----------------");
			if (dbRequestS("LURET")) {
				GoalLPpub(tx,ty,rz,rw);
				pos.linear.z = 42.7; // similiar to post move without scan matching
				pos.angular.z = 1.0; // turn to orientation ? 0.0 = no
			...
			break;
		case 25: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign LULB -----------------");
			if (dbRequestS("LULB")) {
				calcQuatLP("LULB","LBLBAI");
				GoalLPpub(tx,ty,rz,rw);
				pos.linear.z = 42.7; // similiar to post move without scan matching
				pos.angular.z = 1.0; // turn to orientation ? 0.0 = no
				pos_pub.publish(pos);
			...
			break;
		case 26: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign LULBM -----------------");
			if (dbRequestS("LULBM")) {
				GoalLPpub(tx,ty,rz,rw);
				pos.linear.z = 42.6; // similiar to post move without scan matching
				pos.angular.z = 0.0; // turn to orientation ? 0.0 = no
				pos_pub.publish(pos);
			}
			break;
		case 27: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign LBLBAI -----------------");
			if (dbRequestS("LBLBAI")) {
				calcQuatLP("LBLBAI","LBLBBI");
				GoalLPpub(tx,ty,rz,rw);
				pos.linear.z = 42.7; // similiar to post move without scan matching
				pos.angular.z = 1.0; // turn to orientation ? 0.0 = no
				pos_pub.publish(pos);
			...
			break;
		case 28: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign LBLBBI -----------------");
			if (dbRequestS("LBLBBI")) {
				calcQuatLP("LBLBBI","LBLD");
				GoalLPpub(tx,ty,rz,rw);
				pos.linear.z = 42.7; // similiar to post move without scan matching
				pos.angular.z = 1.0; // turn to orientation ? 0.0 = no
				pos_pub.publish(pos);
			...
			break;
		case 29: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign LBLD -----------------");
			if (dbRequestS("LBLD")) {
				GoalLPpub(tx,ty,rz,rw);
				pos.linear.z = 42.7; // similiar to post move without scan matching
				pos.angular.z = 1.0; // turn to orientation ? 0.0 = no
			...
			break;
		case 30: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign LFLD -----------------");
			if (dbRequestS("LFLD")) {
				GoalLPpub(tx,ty,rz,rw);
				pos.linear.z = 42.7; // similiar to post move without scan matching
				pos.angular.z = 1.0; // turn to orientation ? 0.0 = no
			...
			break;
		case 31: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign LFIN -----------------");
			if (dbRequestS("LFIN")) {
				GoalLPpub(tx,ty,rz,rw);
				pos.linear.z = 42.7; // similiar to post move without scan matching
				pos.angular.z = 1.0; // turn to orientation ? 0.0 = no
		...
		case 32:
			...
			break;
		case 33: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign DOCK -----------------");
			if (dbRequestS("DOCK")) {
				GoalLPpub(tx,ty,rz,rw);
				pos.linear.z = 42.7; // similiar to post move without scan matching
				pos.angular.z = 1.0; // turn to orientation ? 0.0 = no
			...
			break;
		case 34: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign LBLBAIM -----------------");
			if (dbRequestS("LBLBAIM")) {
				GoalLPpub(tx,ty,rz,rw);
				pos.linear.z = 42.7; // similiar to post move without scan matching
				pos.angular.z = 1.0; // turn to orientation ? 0.0 = no
				pos_pub.publish(pos);
			...
			break;
		case 35: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign LBLD -----------------");
			if (dbRequestS("LBLD")) {
				GoalLPpub(tx,ty,rz,rw);
				pos.linear.z = 42.7; // similiar to post move without scan matching
			...
			break;
		case 36: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign LFLBD -----------------");
			if (dbRequestS("LFLBD")) {
				GoalLPpub(tx,ty,rz,rw);
				pos.linear.z = 42.7; // similiar to post move without scan matching
				pos.angular.z = 0.0; // turn to orientation ? 0.0 = no
			...
			break;
		case 37: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign L1LRm3 -----------------");
			if (dbRequestS("L1LRm3")) {
				GoalLPpub(tx,ty,rz,rw);
				pos.linear.z = 42.7; // similiar to post move without scan matching
				pos.angular.z = 1.0; // turn to orientation ? 0.0 = no
				pos_pub.publish(pos);
			...
			break;
		case 38: // test moving to LP and turn to orientation
			ROS_INFO("---------- mNode : ptpMoveAlign scanmatching -----------------");
			scanMatchAlign(1);
			break;
		case 39: // test path following
			ROS_INFO("---------- mNode : pathFollowing LBLBAIM -----------------");
			pos.linear.z = 42.8; // similiar to post move without scan matching
			pos.angular.z = 1.0; // path1
			pos_pub.publish(pos);
			break;
		case 300:
			ntucLobbyDoor = 1;
...
void MovNode::checkDockLP(std::string lps) {
	...
	if (s1.compare(lps) == 0) {
		dockLP = true;
		ROS_INFO("--- checkDockLP : LP is Docking Station ---");
	} else {
		dockLP = false;
		ROS_INFO("--- checkDockLP : LP is NOT Docking Station ---");
	}
}
...
bool MovNode::dbRequestT(std::string lps) {
	bool ret;
	std::string s1;
	...
	//ROS_INFO("---------- MoveNode : dbRequestT. Point : %s. -------",lps.c_str());
	if (web_client.call(web_srv)) {
		//ROS_INFO("--------- MoveNode Received reply from dbase Server");
		if (web_srv.response.status == 30) {	 
			wtx = web_srv.response.tx;   	
			wty = web_srv.response.ty;
			wtz = web_srv.response.tz;
			wrx = web_srv.response.rx;   	
			wry = web_srv.response.ry;
			wrz = web_srv.response.rz;
			wrw = web_srv.response.rw;
			ret = true;
		} else {
	    ROS_INFO("------------- MovNode : Failed to call web service. -------------");				
			ret = false;
		...
		ret = false;
		ROS_INFO("-------- MoveNode : dbRequestT Error---------");
	}
	//ROS_INFO("-------- MoveNode : dbRequestS Exit ---------");
	return ret;
}
...
void MovNode::moveLPLoop()
{
	...
	switch (mstate) {
		case 0: // no movement state					
			...
			rn.getParam("NTUCDOCK",NTUCDOCK);		
			rn.getParam("ONCEMOVEREADY",ONCEMOVEREADY);	
					...
					if (!ONCEMOVEREADY) {
						if (MOVEREADY) {
							if ( ros::Time::now() > (clearmap_start_time + ros::Duration(5.0) )) {
								ntucjobstatus = 0;
								publish_ntuc_status(1,1,targetLP,(int)(avgenergy),ntucjobstatus);
								//ROS_INFO("---------mNode : Sending AGV Started and Ready ------------");
		...
		case 10:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				...
				if ((align == 0)) {  // no alignment
					...
					mstate = 15;
					ROS_INFO("---------- MoveNode : No alignment ---------");
				} else {					
					publish_pose();
					if (align == 1.0) {  // dx,dy
		...
		case 150:  // post move
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if ((post_dist == ZSD) && (post_angle == ZSA) && (post_dist1 == ZSD) && (post_angle1 == ZSA)) {
					movedone = 0;
					moveflag = 1;
					mstate = 15;
					clearmap_start_time = ros::Time::now();
		...
		case 151:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						if ((post_dist1 == ZSD) && (post_angle1 == ZSA)) {
							movedone = 0;
							moveflag = 1;
							mstate = 15;
							clearmap_start_time = ros::Time::now();
		...
		case 152:
			if (dockLP) {
				/*
				ROS_INFO("---------- mNode 152 : Align to DOCK -----------------");
				if (dbRequestS("DOCK")) {
					movedone = 1;
					GoalLPpub(tx,ty,rz,rw);
					pos.linear.z = 42.6; // similiar to post move without scan matching
		...
		case 161:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				...
				pos_pub.publish(pos);
				mstate = 162;
				ROS_INFO("------------- MovNode 161 : Rotate -90 . -----------");
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 162:
		...
		case 16201:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				...
				moveflag = 0;
				pos.linear.z = 42.2; // 
				pos.angular.z = -findtrolleyretreat((int)trolley); // reverse trolley dist
				pos_pub.publish(pos);
				mstate = 162; //166;
				clearmap_start_time = ros::Time::now();
				ROS_INFO("----------- MovNode 16201 : Bring Trolley out with reversing to retry alignment and pickup -----------");
			}
			break;
		case 1621:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.4)) ) {
				// rotate 90
				movedone = 1;
				moveflag = 0;
				pos.linear.z = 42.0; // 
				...
				pos_pub.publish(pos);
				mstate = 1622;
				ROS_INFO("------------- MovNode 1621 : Rotate 90 . -----------");
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 1622:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						// vision alignment
						ROS_INFO("-------- MovNode 1622 : Vision Alignment. ----------");
						movedone = 1;
						moveflag = 0;
						pos.linear.z = 42.5; // 
						//pos.linear.x = trolley; //  trolley type
		...
		case 1623:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.4)) ) {
				...
				pos_pub.publish(pos);
				mstate = 163;
				ROS_INFO("------------- MovNode 1623 : Rotate -90 . -----------");
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 163:
		...
		case 168:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				...
				pos_pub.publish(pos);
				mstate = 1690; //169;
				ROS_INFO("------------- MovNode 168 : Rotate 90 . -----------");
			}
			break;
		case 169:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
		...
		case 1690:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						// vision alignment
						ROS_INFO("-------- MovNode 1690 : Vision Alignment. ----------");
						movedone = 1;
						moveflag = 0;
						pos.linear.z = 42.5; // 
						//pos.linear.x = trolley; //  trolley type
		...
		case 1692:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (agvjackstatus) {
					mstate = 1695; //1693;			
					ROS_INFO("---------- MovNode 1692 : Jack is Up. -------------");
					AGVCARRYTROLLEY = true;
				}
				clearmap_start_time = ros::Time::now();
			}
		...
		case 1693:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				...
				pos_pub.publish(pos);
				mstate = 1694;
				ROS_INFO("------------- MovNode 1693 : Rotate 90 . -----------");
			}
			break;
		case 1694:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
		...
		case 1695:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				...
				publish_status("----------- MovNode 1695 : PickUp Completed. -----------");
				ntucjobstatus = 3; // not ready
				publish_ntuc_status(1,1,targetLP,(int)(avgenergy),ntucjobstatus);
			}
			break;
		...
		case 180:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						rn.getParam("FrontObsDist",ProfileMoveObsDist);
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						...
						ROS_INFO("------- mNodeRet 188 : Job Done. ----------");
						publish_status("------- mNodeRet 188 : Dropped Trolley. Job Done. ----------");
					}
				}
				clearmap_start_time = ros::Time::now();
			}
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						publish_sound(STOPMOVESOUND,0,0);	
						checkDockLP(cLPName);
						ROS_INFO("-------- mNode 40 : Reached Destination. Performing Alignment ----------");
						publish_move_complete("move completed");
						lobbyDoorOpen = false;
						if ((align == 0)) {  // no alignment
							ROS_INFO("----------- MoveNode 40 : No Align . Go 150 --------------------");
							mstate = 150;
						} else {					
							if (align == 2.0) {  
								scanMatchAlign(1);
								mstate = 41;
								movedone = 1;	
								ROS_INFO("----------- MoveNode 40 : Scan Align one time. Go 41 --------------------");		
							} else {
								if (align == 3.0) {  // scan match							
									scanMatchAlign(1);// 3
									mstate = 42;
									movedone = 1;	
									ROS_INFO("----------- MoveNode 40 : Scan Align first time. Go 42 --------------------");		
								}
							}			
		...
		case 42:
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						ROS_INFO("----------- MNode 42 : Scan Align second time. go 43 --------------------");				
						scanMatchAlign(3);
						movedone = 1;
						mstate = 43; 
						clearmap_start_time = ros::Time::now();
		...
		case 2052: // LIFTAREA_TO_LAUNDRYAREA : Taking Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.05)) ) {
				targetfloor = 1;
				fstate = 20520;
				mstate = 4050; // ask for lift 
				clearmap_start_time = ros::Time::now();
			}
			break;		
		case 20520: // LIFTAREA_TO_LAUNDRYAREA : Taking Lift
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.05)) ) {
				targetfloor = 1;
				fstate = 2053;
				mstate = 4051; // lift taking
				clearmap_start_time = ros::Time::now();
			}
			break;
		...
		case 2055:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (dbRequestS("LBLD")) {
					...
					nstate = 20550;//4111;				
					publish_sound(SSMC,0,0);	
					//publish_status("--- MovNode 2055 : Move To LBLB  ----");
				} else {
					ROS_INFO("--------- MovNode 2055 : LoadingBay_lobby LP  Error. -----------");
				}			
			}
			break;
		case 20550: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LBLB")) {
					publish_clear();
					movedone = 0;
					...
					nstate = 2056;
					publish_sound(SSMC,0,0);
					LoadingBayStatus = true;
					rn.setParam("LoadingBayStatus",LoadingBayStatus);
				} else {
					ROS_INFO("--------- MovNode 20550 : LBLB LP  Error. -----------");
				}		
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		case 2057:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LBLBBI")) {
					...
					nstate = 20570;
					publish_sound(SSMC,0,0);
					//publish_status("--- MovNode 2057 : Move To LBLBBI  ----");
				} else {
					ROS_INFO("---------MovNode 2057 :  LBLBBI LP  Error. -----------");
				}
			}
			break;
		case 20570:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LBLBAIM")) {
					...
					nstate = 20571;
					publish_sound(SSMC,0,0);
					//publish_status("--- MovNode 20570 : Move To LBLBAIMB  ----");
				} else {
					ROS_INFO("---------MovNode 20570 :  LBLBAIMB LP  Error. -----------");
				}
			}
			break;
		case 20571:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LBLBAI")) {
					...
					nstate = 20573;
					publish_sound(SSMC,0,0);
					//publish_status("--- MovNode 20571 : Move To LBLBAIB  ----");
				} else {
					ROS_INFO("---------MovNode 20571 :  LBLBAIB LP  Error. -----------");
				}
			}
			break;
		case 20572:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LULB")) {
					...
					nstate = 20573;
					publish_sound(SSMC,0,0);
					publish_status("--- MovNode 20572 : Move To LULB  ----");
				} else {
					ROS_INFO("---------MovNode 20572 :  LULB LP  Error. -----------");
				}
			}
			break;
		case 20573: // LIFTAREA_TO_LAUNDRYAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LURET")) {
					movedone = 0;
					moveflag = 1;
					...
					ROS_INFO("---------MovNode 20573 :  LURET LP  Error. -----------");
				}
			}
			break;
		case 20574:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LUENT")) {
					...
					nstate = 2058;
					publish_sound(SSMC,0,0);
					//publish_status("--- MovNode 20574 : Move To LUENT  ----");
				} else {
					ROS_INFO("---------MovNode 20574 :  LUENT LP  Error. -----------");
				}
			}
			break;
		case 20575: // 
		...
		case 2062:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				ROS_INFO("---------- MovNode 22013 : ptpmovealign to LUENT. ----------");
				GoalLPpub(tx,ty,rz,rw);
				movedone = 1;
				moveflag = 0;
				pos.linear.z = 42.6; // 
		...
		case 2063:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				...
				if (ret) { 
					...
					ROS_INFO("---------MovNode 2063 :  DOCK LP  Error. -----------");
				}	
				clearmap_start_time = ros::Time::now();
			}
			break;
		...
		case 21020:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LBLBBI")) {
					...
					nstate = 21021;
					publish_sound(SSMC,0,0);
					publish_status("--- MovNode 41120 : Move To LBLBBIB  ----");
				} else {
					ROS_INFO("---------MovNode 41120 :  LURET LP  Error. -----------");
				}
			}
			break;
		case 21021:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LBLBAIM")) {
					...
					nstate = 41122;
					publish_sound(SSMC,0,0);
					publish_status("--- MovNode 21021 : Move To LBLBAIMB  ----");
				} else {
					ROS_INFO("---------MovNode 21021 :  LBLBAIMB LP  Error. -----------");
				}
			}
			break;
		case 21022:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LBLBAI")) {
					...
					nstate = 2103; //41123;
					publish_sound(SSMC,0,0);
					publish_status("--- MovNode 21022 : Move To LBLBAIB  ----");
				} else {
					ROS_INFO("---------MovNode 21022 :  LBLBAIB LP  Error. -----------");
				}
			}
			break;
		...
		case 21030:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LUENT")) {
					...
					nstate = 2104;
					publish_sound(SSMC,0,0);
					publish_status("--- MovNode 21030 : Move To LUENT  ----");
				} else {
					ROS_INFO("---------MovNode 21030 :  LUENT LP  Error. -----------");
				}
			}
			break;
		case 21031: // 
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				...
				mstate = 21031;
				clearmap_start_time = ros::Time::now();
				ROS_INFO("----------- MovNode 21031 : Moving through Laundry Door into Laundry. -----------");
				publish_status("--- MovNode 21031 : Move through Laundry Door  ----");
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 21032: 
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {					
						mstate = 2104;
						ROS_INFO("-------- MovNode 21032 : Ready to move to Laundry area. -----------");
					}
				}
				clearmap_start_time = ros::Time::now();	
			}
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.05)) ) {
				if (!agvjackUp) {
					...
					if (ret) { 
						...
						ROS_INFO("---------MovNode 2107 : Dock LP  Error. -----------");
					}
				}
				clearmap_start_time = ros::Time::now();	
			}
		...
		case 22013: // use ptpmovealign
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (dbRequestS("LUENT")) {
					...
					ROS_INFO("---------MovNode 22013 : LUENT LP  Error -----------");
				}
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 22014: 
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LURET")) {		
					...
					ROS_INFO("---------MovNode 22014 : LURET LP  Error -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		*/
		...
		case 22030: 
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.05)) ) {
				if (dbRequestS("LBLBAI")) {	
					...
					ROS_INFO("---------MovNode 22030 : LBLBAI LP  Error -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 22031: 
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LBLBAIM")) {		
					...
					ROS_INFO("---------MovNode 22014 : LBLBAIM LP  Error -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 22032: 
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LBLBBI")) {		
					...
					ROS_INFO("---------MovNode 22014 : LBLBBI LP  Error -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		case 22050:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.05)) ) {
				currentfloor = 1;	
				publish_status("--- MovNode 22050 : Ask for Lift  ----");
				mstate = 4050;
				fstate = 22051; //2206;
				//publish_sound(ASKLIFT,0,0);
				//liftDoorOpen = false;				
				//publish_status("------------- MovNode 22050 : AGV Requesting for Lift  ----------------");
		...
		case 22051: 
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (dbRequestS("LFLDN")) {	//lFlBD
					calcQuatLP("LFLDN","LFLD");	
					movedone = 0;			
					moveflag = 1;	
					mstate = 4;
					align = 0.0;
					...
					ROS_INFO("---------MovNode 22051 : LFLBD LP  Error -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		*/
		...
		case 30030: 
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.05)) ) {
				if (dbRequestS("LBLBAI")) {	
					...
					ROS_INFO("---------MovNode 30030 : LBLBAI LP  Error -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 30031: 
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LBLBAIM")) {		
					...
					ROS_INFO("---------MovNode 30031 : LBLBAIM LP  Error -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 30032: 
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LBLBBI")) {		
					...
					ROS_INFO("---------MovNode 30032 : LBLBBI LP  Error -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		case 30033: 
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LBLB")) {		
					calcQuatLP("LBLB","LBLD");
					mstate = 4;
					align = 0.0;
					redirect = true;
					nstate = 	3004;
				} else {
					ROS_INFO("---------MovNode 30033 : LBLB LP  Error -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
		...
		case 3050: // LIFTAREA_TO_LIFTAREA
			...
			publish_status("----------- MovNode 3050 : LIFTAREA_TO_LIFTAREA ---------------");
			//mstate = 3500; // pre-move
			//nstate = 3051;
			movedone = 0;
			moveflag = 1;
			...
			//publish_sound(ASKLIFT,0,0);
			//liftDoorOpen = false;
			//ROS_INFO("-------------MovNode 3050 : AGV Requesting for Lift  ----------------");
			//publish_status("------------- MovNode 3050 : AGV Requesting for Lift  ----------------");
			//publish_lift(ASKLIFTTOCOME,currentfloor,0,1,0);
			break;
				...
				if (movedone == 0) {
					if (moveflag == 1) {
						if (dbRequestSP("LFLD")) {
							...
							nstate = 3052;					
							publish_sound(SSMC,0,0);		
							ROS_INFO("--------- MovNode 3051 : move to LFLD -----------");
							publish_status("--------- MovNode 3051 : move to LFLD -----------");
						} else {
		...
		case 3054: // LIFTAREA_TO_LIFTAREA
			...
			break;
		case 3055: // go to LFLD
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LFLD")) {
					publish_status("--- MovNode 3055 : Move To LFLD  ----");
					ROS_INFO("---------- MovNode 3055 : Move to LFLD. --------");
					movedone = 0;
					moveflag = 1;											
					redirect = true;		
					align = 3.0;				
					...
					ROS_INFO("---------MovNode 3055 :  LFLD LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		case 4050: // Lift Taking
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.8)) ) {
				...
				publish_status("------------- MovNode 4050 : AGV Requesting for Lift  ----------------");
				publish_lift(ASKLIFTTOCOME,currentfloor,0,1,0);
				//mstate = 4051;
				mstate = fstate;
				clearmap_start_time = ros::Time::now();
		...
		case 4051: // Lift Taking
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if ((ntucstatus == 10) && (liftDoorOpen)) {
					...
					liftDoorOpen = false;
					mstate = 4052;			
					//dynaReconfig(global_costmap_inflation_radius,1,0.4,"");		
					//publish_sound(LIFTARRIVED,0,0);					
					publish_sound(LIFTARRIVEDMOVEINTOLIFT,0,0);
					//publish_clear();
					ROS_INFO("------------- MovNode 4051 : lift arrived ----------------");			
					publish_status("------------- MovNode 4051 : lift arrived ----------------");	
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		case 4053: // Lift Taking
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				...
				publish_status("--- MovNode 4053 : AGV Moving into lift ----");						
				if (dbRequestS("LFIN")) {
					//publish_sound(MOVEINTOLIFT,0,0);
					movedone = 0;			
					moveflag = 1;						
		...
		case 4055: // Lift Taking
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				if (!agvjackstatus) {
					// AGV rotate 180.0
					//ret = system("sudo service network-manager restart &");  // restart network manager
			...
			break;
		case 40550: // Lift Taking
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {	
						if (AGVCARRYTROLLEY) {
							// vision alignment
							//ROS_INFO("-------- MovNode 40550 : Vision Alignment. ----------");
							//movedone = 1;
							//moveflag = 0;
							//pos.linear.z = 42.5; // 
							//pos_pub.publish(pos);
		...
		case 4058: // Lift Taking
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {	
				...
				publish_status("---------- MovNode 4058 : Ask for lift. ----------");	
				//restoreLP();	
				publish_sound(ASKLIFT,0,0);
		...
		case 4059: // Lift Taking
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				if ((ntucstatus == 11) && (liftDoorOpen)) {
					...
					mstate = 4060; // 		
					//publish_sound(LIFTARRIVED,0,0);								
					publish_sound(LIFTDOOROPENMOVEOUT,0,0);	
		...
		case 4061: // Lift Taking
			...
			break;
		case 4062: // Lift Taking
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(10.0)) ) {
				mstate = 4058;
				ROS_INFO("---------MovNode 4062 : Ask for lift before jackup complete -----------");
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		case 4103: // 
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				targetfloor = 1;
				fstate = 41030;
				mstate = 4050; // ask for lift taking
				//publish_status("--- MovNode 4103 : ask for lift Lift  ----");
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 41030: // 
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				...
				fstate = 4104;
				mstate = 4051; // lift taking
				publish_status("--- MovNode 41030 : Taking Lift  ----");
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 4104://
		...
		case 41121:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LBLBAIM")) {
					...
					nstate = 41122;
					publish_sound(SSMC,0,0);
					publish_status("--- MovNode 41121 : Move To LBLBAIMB  ----");
				} else {
					ROS_INFO("---------MovNode 41121 :  LBLBAIMB LP  Error. -----------");
				}
			}
			break;
		case 41122:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LBLBAI")) {
					...
					nstate = 4113; //41123;
					publish_sound(SSMC,0,0);
					publish_status("--- MovNode 41122 : Move To LBLBAIB  ----");
				} else {
					ROS_INFO("---------MovNode 41122 :  LBLBAIB LP  Error. -----------");
				}
			}
			break;
		case 41123:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LULB")) {
					...
					nstate = 4113;
					publish_sound(SSMC,0,0);
					publish_status("--- MovNode 41123 : Move To LULBB  ----");
				} else {
					ROS_INFO("---------MovNode 41123 :  LULBB LP  Error. -----------");
				}
			}
			break;
		case 4113:
		...
		case 41132:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				if (dbRequestS("LUENT")) {
					...
					nstate = 4114;
					publish_sound(SSMC,0,0);
					publish_status("--- MovNode 41132 : Move To LUENT  ----");
				} else {
					ROS_INFO("---------MovNode 41132 :  LUENT LP  Error. -----------");
				}
			}
			break;
		case 4114:
		...
		case 5000:
			if (dbRequestSP("L1LRm3")) {
				...
				ROS_INFO("--------- MovNode 5000 : L1LRm3 Error -----------");
			}
			clearmap_start_time = ros::Time::now();
			break;
		case 5001:

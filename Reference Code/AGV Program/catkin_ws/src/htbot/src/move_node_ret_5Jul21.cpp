...
MovNode::MovNode():
  ...
  currentfloor(1),ACTIVATEJACK(false),AGVCARRYTROLLEY(false),OutsideLift(false),lobbyDoorOpen(false),
  jackworking(false),jackComm(true),rstate(0),LaundryDoorGapOut(1.05),LaundryDoorGapIn(0.9)
{
	ros::NodeHandle nh;
	...
	switch (mstate) {
		...
		case 1691:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				...
				//if (destPoint == 2) {
					mstate = nstate;
					redirect = false;
					ROS_INFO("------- mNode 1691 : mstate = %d ------------",mstate);
				} else {
					mstate = 0;
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						...
						ROS_INFO("------------ MovNode 180 : Dist to wall = %.3f. -------------",(ProfileMoveObsDist-0.216));
						mstate = 181;
					}
				}
				clearmap_start_time = ros::Time::now();
		...
		case 181:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.1)) ) {
				...
				moveflag = 0;
				pos.linear.z = 42.2; // 
				//pos.angular.z = ProfileMoveObsDist-0.416 ; //trolleydist; // move trolley dist
				pos.angular.z = trolleydist; // move trolley dist
				pos_pub.publish(pos);
				mstate = 182;				
				ROS_INFO("----- mNoveret 181 : Start of dropping trolley. Move straight = %.3f --------------",(ProfileMoveObsDist-0.416));
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 182:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						...
						pos_pub.publish(pos);
						mstate = 183;
						ROS_INFO("----- mNoveret 182 : Start of dropping trolley. Turn 90 --------------");
					}
				}
				clearmap_start_time = ros::Time::now();
			}
		...
		case 186:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						rn.getParam("ProfileMoveObsDist",ProfileMoveObsDist);
						ROS_INFO("------------ MovNode 186 : Dist Clear to move out of trolley = %.3f. -------------",(ProfileMoveObsDist-0.216));
						if ((ProfileMoveObsDist-0.216) > 1.0) {
							mstate = 187;
						} else {
		...
		case 187:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				...
				moveflag = 0;
				pos.linear.z = 42.2; // 
				pos.angular.z = trolleydist + 0.15; // move trolley dist
				pos_pub.publish(pos);
				mstate = 188;
				clearmap_start_time = ros::Time::now();
			}

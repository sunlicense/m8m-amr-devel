...
*		Changes :
*   30.8.21 : 
			1. 3.20pm : create new right laser obstacle distance calculation
*/
...
void laserRCallBack(const sensor_msgs::LaserScan::ConstPtr& lscan) {	
	...
	if (od > 0.0) {
		nx.setParam("RightObsDist",od);					
	} else {
		nx.setParam("RightObsDist",4.0);
	}
}
...
{
	ros::init(argc, argv, "Right_laser_filter");
  ros::NodeHandle n;
	
	flag = false;
	laserR_sub= n.subscribe<sensor_msgs::LaserScan>("/scanRt",100, laserRCallBack);	 // /scan_base - org

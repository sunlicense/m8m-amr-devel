	...
	switch (mstate) {
		...
		case 1621:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.4)) ) {
				...
				pos_pub.publish(pos);
				mstate = 1622;
				ROS_INFO("------------- MovNode 1621 : Rotate 90 Clockwise. -----------");
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 1622:
		...
		case 1623:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.4)) ) {
				...
				pos_pub.publish(pos);
				mstate = 163;
				ROS_INFO("------------- MovNode 1623 : Rotate -90 Clockwise. -----------");
				clearmap_start_time = ros::Time::now();
			}
			break;
		case 163:
					...
					if (moveflag == 1) {
						...
						if ((align == 0)) {  // no alignment
							...
							if (align == 2.0) {  
								scanMatchAlign(1);
								mstate = 41;
								ROS_INFO("----------- MoveNode 41 : Scan Align one time --------------------");		
							} else {
								if (align == 3.0) {  // scan match							
									scanMatchAlign(1);// 3
									mstate = 42;
									ROS_INFO("----------- MoveNode 41 : Scan Align first time --------------------");		
								}
							}			
							movedone = 1;			
						}
		...
		case 42:
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						ROS_INFO("----------- MNode 42 : Scan Align second time --------------------");				
						scanMatchAlign(3);
						movedone = 1;
						mstate = 43; 
						clearmap_start_time = ros::Time::now();
		...
		case 2153: // LOBBYAREA_TO_LIFTAREA
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.3)) ) {
				if (dbRequestS("LFLD")) {
					publish_clear();
					movedone = 0;					
					moveflag = 1;
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {			
						if (dbRequestS("LURET")) {		
							GoalLPpub(tx,ty,rz,rw);
							ROS_INFO("---------- MovNode 22014 : ptpmovealign to LURET. ----------");
							movedone = 1;
							moveflag = 0;
							pos.linear.z = 42.6; // 
							pos.angular.z = 1.0;
		...
		*/
		...
		case 4061: // Lift Taking
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				...
				//dynaReconfig(global_costmap_inflation_radius,1,0.48,"");		
				mstate = fstate;

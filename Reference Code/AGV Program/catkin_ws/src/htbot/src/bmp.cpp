...
BMPImage *crop_bmp(BMPImage *image, int x, int y, int w, int h, char **error)
{
    ...
    // Allocate memory for image data
    new_image->data = (unsigned char*)malloc(sizeof(*new_image->data) * new_image->header.image_size_bytes);
...
void locate(BMPImage *image)
{
	int position_y = _get_image_row_size_bytes(&image->header);
  int position_x_row = _get_position_x_row(0, &image->header);
	int width,height,yw,xp,yp;
	BMPHeader *bh;
  ...
  for (int i = 0; i < height; i++) {
    ...
    for (int j = 0; j < width; j++) {
    	// Iterate image's pixels
			xp = _get_position_x_row(j, &image->header);
			bb = image->data[yp + xp];
			gg = image->data[yp + xp + 1];
			rr = image->data[yp + xp + 2];
			if ((bb < 0x20) && (gg < 0x20) && (rr > 0xf0)) {
				ROS_INFO("---- BMP : x=%d. y=%d : bb=%x. gg=%x. rr=%x. ---------",j,i,bb,gg,rr);
			}
  	}
		yp += yw;        
	}
...
int main(int argc, char **argv)
{
  ...
  if(fp==NULL){
	...
	fp1 = fopen("/home/rac/catkin_ws/src/htbot/maps/testn.bmp", "w");
  if(fp1==NULL){
  	ROS_INFO("-------- BMP : Failed to Open Testn.bmp ----------------");
		return -1;
  }
	size = _get_file_size(fp);
	ROS_INFO("-------- BMP : File Size=%ld ----------------",size);
	bimg = read_bmp(fp,err);
	bh = &bimg->header;
	ret = check_bmp_header(bh,fp);
	if (ret) {
		ROS_INFO("-------- BMP : Header is OK ----------------");
		ROS_INFO("-- BMP : type=%x. Size=%d. BitsperPixe=%d. width=%d. height=%d. nclr=%d --",bh->type,bh->size,bh->bits_per_pixel,bh->width_px,bh->height_px,bh->num_colors);
		xp = _get_position_x_row(0,bh);
		yw = _get_image_row_size_bytes(bh);
		ROS_INFO("----- xp=%d. yw=%d-----------",xp,yw);
	} else {
		ROS_INFO("-------- BMP : Header is Bad ----------------");
		return -1;
	}
	// 
	xp = _get_position_x_row(0,bh);
	...
	ROS_INFO("--- Pixel@357,210 : bb=%x. gg=%x. rr=%x. ----",bb,gg,rr);
	nimg = crop_bmp(bimg,279,129,50,30,err);
	ret = write_bmp(fp1,nimg,err);
	locate(bimg);
	free_bmp(bimg);
	free_bmp(nimg);
	fclose(fp);

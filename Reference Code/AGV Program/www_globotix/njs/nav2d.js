...
NAV2D.Navigator = function(options) {
  ...
  if (that.rootObject instanceof createjs.Stage) {
	...
	var wlaser = new createjs.Shape();	
	stage.addChild(wlaser);
	stage.snapToPixelEnabked = true;
	var poseDir;
  ...
  poseListener.subscribe(function(pose) {
	...
	// setup a listener for the base laser scan
	
  var laserListener = new ROSLIB.Topic({
    ros : ros,
    name : '/weblaser',
    messageType : 'htbot/weblaser',
    throttle_rate : 10
  });
  laserListener.subscribe(function(scan) {
    // update the robots position on the map
    var size = scan.size;
		var i;
		//wlaser.graphics.setStrokeStyle(1);	
  	//wlaser.graphics.beginStroke("#ff0000");
		wlaser.graphics.clear()
		wlaser.graphics.beginFill("#ff0000");
		for (i=0;i<size;i++) {
			wlaser.graphics.moveTo(scan.px[i],-scan.py[i]);
			wlaser.graphics.drawCircle(scan.px[i],-scan.py[i],0.06);
		}
		wlaser.graphics.endFill();
  	wlaser.graphics.closePath();  
  	wlaser.graphics.endStroke();
		//stage.update();
  });
...
NAV2D.AGVNYS = function(options) {
  ...
  if (that.rootObject instanceof createjs.Stage) {
	...
	var poseDir;
  // marker for the robot
  var robotMarker = new ROS2D.RobotVehicle({
    size : 10,
    strokeSize : 1,
    //fillColor : createjs.Graphics.getRGB(255, 128, 0, 0.66),
		fillColor : createjs.Graphics.getRGB(255, 0, 0, 0.66),
...
NAV2D.OccupancyGridClientNavNYS = function(options) {
  var that = this;
  options = options || {};
  this.ros = options.ros;
  var topic = options.topic || '/map';
  ...
  client.on('change', function() {
    that.navigator = new NAV2D.AGVNYS({
      ros : that.ros,
      rootObject : that.rootObject,
    });

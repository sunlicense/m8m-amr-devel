...
/**
 * A RobotVehicle is a directed vehicle that can be used to display orientation.
 *
 * @constructor
 * @param options - object with following keys:
 *   * size (optional) - the size of the marker
 ...
 *   * pulse (optional) - if the marker should "pulse" over time
 */
ROS2D.RobotVehicle = function(options) {
  var that = this;
  options = options || {};
  var size = options.size || 10;
  var strokeSize = options.strokeSize || 3;
...
ROS2D.RobotVehicle.prototype.__proto__ = createjs.Shape.prototype;
...
ROS2D.Viewer = function(options) {
  ...
  var canvas = document.createElement('canvas');
  canvas.width = this.width; //window.innerWidth; //this.width;
	//this.width = window.innerWidth;
  canvas.height = this.height; //window.innerHeight; //this.height;

...
lift_status_pub = rospy.Publisher('lift_status_from_FM', UInt8, queue_size=100)
...
##### ##### ##### ##### ##### ##### ##### ##### #####
varLiftStatus = root.get_child(["0:Objects", "2:Parameters", "2:LiftStatus"])		
...
##### ##### ##### ##### ##### ##### ##### #####
##### Callback for ROS topic /lift_status, std_msgs/UInt8 #####
##### Message sent by lift-proxy to update lift status on Fleet Manager #####
##### Currently not used yet #####
##### ##### ##### ##### ##### ##### ##### #####
def fr_lift_proxy_callback(msg):
	print("Callback data received msg.data==%d" % msg.data)
	varLiftStatus.set_value(msg.data)

rospy.Subscriber('lift_status_to_FM', UInt8, fr_lift_proxy_callback) 
...
if __name__ == '__main__':
	...
	curLiftStatus=0			## current lift status	
	...
	while True:
		...
		try:
			...
			newLiftStatus = varLiftStatus.get_value()
		...
		##### ##### ##### ##### ##### ##### ##### #####  ##### 	
		### Publish new lift status to lift-proxy
		##### ##### ##### ##### ##### ##### ##### #####  ##### 
		if (curLiftStatus != newLiftStatus):	## lift status on Fleet Manager changes
			if newLiftStatus:				## newLiftStatus!=0
				rospy.loginfo("--- Lift is occupied by AGV%d ---", newLiftStatus)
				
			pub_msg = UInt8()				### an instant of std_msgs/UInt8 message type
			pub_msg.data = newLiftStatus
			lift_status_pub.publish(pub_msg)
			curLiftStatus=newLiftStatus
			 
		##if (loop_count%5==0):	## limit onscreen printing
		...
		if (curTask[0] != newTask[0] or curTask[1] != newTask[1] or
			...
			rospy.loginfo("Fleet --- newTask ... %d, %d, %d, %d, %d ---",newTask[0],newTask[1],newTask[3],newTask[4],newTask[5])
		### Task [cmd, time.secs, type, frLP, toLP, pub2AGV] ###
			if (newTask[0]==2 and newTask[5]==0):	## assigned task from call stations ##
				...
				rospy.loginfo("Fleet --- Published to AGV ... %d, %d, %d, %d, %d ---",newTask[0],newTask[1],newTask[3],newTask[4],newTask[5])

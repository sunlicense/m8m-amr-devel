...
public:
	...
	int numVP;
	bool UPJACK,ACTIVATEJACK,redirect,AGVCARRYTROLLEY;
...
MovNode::MovNode():
  ...
  currentfloor(1),ACTIVATEJACK(false),AGVCARRYTROLLEY(false)
{
	ros::NodeHandle nh;
	...
	switch (mstate) {
		...
		case 3081:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				...
				if (ProfileMoveObsDist > DistClearance) {					
					publish_sound(MOVEOUTOFLIFT,0,0);		
					ROS_INFO("---------- MovNode 3081 : Move out of Lift to Ward . ----------");		
					if (dbRequestS("LFLBD")) {
						movedone = 1;
						moveflag = 0;
						redirect = true;
		...
		case 3600: // check if new job in queue
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(5.0)) ) {
				...
				if ((numLPInQ > 0) && botrun) {
					...
					mstate = nstate;
					ROS_INFO("------------ MovNode 3600 : No Job in queue. mstate=%d ----------",mstate);
				}
			}
			break;
	}

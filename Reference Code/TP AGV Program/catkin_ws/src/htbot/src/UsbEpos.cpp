 ...
 * last update : 26.4.21 3.00pm
	...
	21.5.21 : 9.20am 
		1.change turntopath only for angle greater than 45.0
	...
	4Aug21 
		1. added moveDistanceSpeedRT
	5Aug21 
		1. 12.40pm : changed adjustAGVVision to return true if tag detected and false if tag is not detected
	9.8.21 
		1. 5pm : added pathfollowing.
	8.9.21 
		1. 5pm : added left and right laser obs dist to moveDistanceSpeedRT
...
public:
	...
	bool adjustAGVVision(void);
	void ptpmoveAlign();
	void moveDistanceSpeedRTTurn(void);
	void moveDistanceSpeedRTSetup(double dist,int velocity);
	void moveDistanceSpeedRTReset(double dist,int velocity);
	void moveDistanceSpeedRT(void);
	void pathFollowRTSetup(double path);
	void pathFollowRTNP(void);
	void pathFollowRT(void);
	void pathFollowRTTP(void);
	...
	double yawp,yawg,yawStart,yawAlign,RTMyawp;
	...
	double RTMdd;
	bool oriented,SpeedRTStop;
	int obscount;
	double gvdy,MPstatus;
	double movePtInfo[20][8];
	int noMP,MPidx;
	bool MPEndPt;
...
BotNode::BotNode():
  ...
  alignflag(false),multiLP(false),maxautolinear(0.5),maxautoangular(0.5),maxjoyangular_(0.6),maxjoylinear_(0.6),
	pauseRobotMove(false),manualvelflag(0),maxVelocity(0.6),gvdy(0.0)
{
...
bool BotNode::adjustAGVVision(void) {
	ros::NodeHandle n;
	double vdx,vdy,vda;
	int vision_valid;
	std_msgs::Int16 msg;
	...
	n.getParam("vision_dy",vdy);
	gvdy = vdy;
	ROS_INFO("--------- Vision : 2nd : vda=%.3f. vdx=%.3f. gvdy=%.3f. ---------",vda,vdx,gvdy);
...
void BotNode::moveDistanceSpeedRTTurn(void) {
	double an,diff,angle;
	...
	angle = 57.2958 * diff;
	RTMdd = sqrt(((gx-px) * (gx-px)) + ((gy-py) * (gy-py)));
	ROS_INFO("--------- moveDistanceSpeedRTTurn : angle=%.3f. ------------",angle);
	moveAngle(angle);
}

void BotNode::moveDistanceSpeedRTSetup(double dist,int velocity) {
	BOOL Absolute = 0;
	BOOL Immediately = 1;
	double an,diff,angle,dleft,dright;

	tf::Quaternion qp(0.0,0.0,prz,prw);
	RTMyawp = tf::getYaw(qp);

	dist = (long)( RTMdd * dist_factor);
	quickstop = false;
	...
	if (VCS_SetPositionProfile(g_pKey,RMOTOR,velocity,ProfileAcceleration,ProfileDeceleration,&eposErrorCode) == 0) {
		ROS_INFO("--------UsbEpos-moveDistanceSpeedRTSetup : SetPositionProfile_RIGHT Error -------------");
	}	
	if (VCS_SetPositionProfile(g_pKey,LMOTOR,velocity,ProfileAcceleration,ProfileDeceleration,&eposErrorCode) == 0) {
		ROS_INFO("--------UsbEpos-moveDistanceSpeedRTSetup : SetPositionProfile_LEFT Error -------------");
	}	
	if(VCS_MoveToPosition(g_pKey,RMOTOR,dright,Absolute,Immediately,&eposErrorCode) == 0)
	{
		ROS_INFO("--------UsbEpos : MoveToPosition_RIGHT Error -------------");
	}
	if(VCS_MoveToPosition(g_pKey,LMOTOR,dleft,Absolute,Immediately,&eposErrorCode) == 0)
	{
		ROS_INFO("--------UsbEpos : MoveToPosition_LEFT Error -------------");
	}
}

void BotNode::moveDistanceSpeedRTReset(double dist,int velocity) {
	
	if (VCS_SetPositionProfile(g_pKey,RMOTOR,ProfileVelocity,ProfileAcceleration,ProfileDeceleration,&eposErrorCode) == 0) {
		ROS_INFO("--------UsbEpos-moveDistanceSpeedRTReset : SetPositionProfile_RIGHT Error -------------");
	}	
	if (VCS_SetPositionProfile(g_pKey,LMOTOR,ProfileVelocity,ProfileAcceleration,ProfileDeceleration,&eposErrorCode) == 0) {		
		ROS_INFO("--------UsbEpos-moveDistanceSpeedRTReset : SetPositionProfile_LEFT Error -------------");
	}	
	setVelocityMode();
}

void BotNode::moveDistanceSpeedRT(void) {
	ros::NodeHandle nn;
	double fobsdist,lobsdist,robsdist,an,diff,dd,as,ls,angle;
	long dleft,dright,d;	
	
	//nn.getParam("ProfileMoveObsDist",obsdist);
	nn.getParam("FrontObsDist",fobsdist);
	nn.getParam("LeftObsDist",lobsdist);
	nn.getParam("RightObsDist",robsdist);
	if (fobsdist < 0.1) {
		fobsdist = 10.0;
	}
	if (lobsdist < 0.05) {
	...
	// check for direction alignment to target
	dd = sqrt(((gx-px) * (gx-px)) + ((gy-py) * (gy-py)));
	dd = fabs(dd);
	//ROS_INFO("------ moveDistanceSpeedRT : obsdist=%.3f. dd=%.3f. ---------",obsdist,dd);
	if (((fobsdist < dd) && (fobsdist < 0.9)) || (lobsdist < 0.1) || (robsdist < 0.1)) {	
		ROS_INFO("------ moveDistanceSpeedRT : fobsdist=%.3f. lobsdist=%.3f. robsdist=%.3f.  dd=%.3f. ---------",fobsdist,lobsdist,robsdist,dd);
		obscount++;
	...
	as = 0.0;
	if (dd < 0.4) {		
		//moveVelocity(0.09,0.0);	
		ls = 0.06;		
		if (dd < 0.15) {
			moveVelocity(0.0,0.0);
			//ROS_INFO("------ moveDistanceSpeedRT : Stop dd=%.3f. ---------",dd);
			SpeedRTStop = true;
			return;
		}
	}
	...
	//ROS_INFO("----  moveDistanceSpeedRT : ls=%.3f. as=%.3f. -------------",ls,as);
	moveVelocity(ls,as);
}

void BotNode::pathFollowRTSetup(double path) {
	FILE *fp;
	double zx,zy,zz,zrx,zry,zrz,zrw,status;
	int i,pp;
	
	pp = (int)path;
	switch(pp) {
		case 1:
			fp = fopen("/home/rac/catkin_ws/src/htbot/data/mpath1.txt", "r");
  		if (fp == NULL) {
  			ROS_INFO("----- pathFollowRTSetup : I couldn't open mpath1.txt for reading. ---------\n");    		
  			return;
 		 	}
			break;
		case 2:
			fp = fopen("/home/rac/catkin_ws/src/htbot/data/mpath2.txt", "r");
  		if (fp == NULL) {
  			ROS_INFO("----- pathFollowRTSetup : I couldn't open mpath2.txt for reading. ---------\n");    		
  			return;
 		 	}
			break;
	}
	noMP = 0;
	ROS_INFO("------- pathFollowRTSetup : Load the path points ------------");
	while(true) {
		if (fscanf(fp,"%lf %lf %lf %lf %lf %lf %lf %lf\n",&zx,&zy,&zz,&zrx,&zry,&zrz,&zrw,&status) == EOF) {
			break;
		}
		movePtInfo[noMP][0] = zx;
		movePtInfo[noMP][1] = zy;
		movePtInfo[noMP][2] = zz;
		movePtInfo[noMP][3] = zrx;
		movePtInfo[noMP][4] = zry;
		movePtInfo[noMP][5] = zrz;
		movePtInfo[noMP][6] = zrw;
		movePtInfo[noMP][7] = status; // 0.0=normal. 1.0=segment marker. segment end point
		ROS_INFO("--- pathFollowRTSetup : MP=%d : x=%.3f. y=%.3f. Status=%.3f ----",i,zx,zy,status);
		noMP++;		
	}
	MPidx = 0;
	MPEndPt = false;
...
void BotNode::pathFollowRTNP(void) {
	double an,diff,angle;

	gx = movePtInfo[MPidx][0];
	gy = movePtInfo[MPidx][1];
	gz = movePtInfo[MPidx][2];
	grx = movePtInfo[MPidx][3];
	gry = movePtInfo[MPidx][4];
	grz = movePtInfo[MPidx][5];
	grw = movePtInfo[MPidx][6];
	MPstatus = movePtInfo[MPidx][7];
	MPidx++;
	if (MPidx == noMP) {
		MPEndPt = true;
	} else {
	...
	*/
	ROS_INFO("------------ pathFollowRTNP : status=%.3f gx=%.3f. gy=%.3f ----------------",MPstatus,gx,gy);
}

void BotNode::pathFollowRTTP(void) {
	ros::NodeHandle nn;
	double obsdist,an,diff,dd,as,ls,angle;
	long dleft,dright,d;	
	
	nn.getParam("FrontObsDist",obsdist);
	if (obsdist < 0.1) {
		obsdist = 10.0;
	}
	if (quickstop) {
		quickStopMotor();
		ROS_INFO("--------- pathFollowRTTP : quickstop. ----------");
		return;
	}
	// check for direction alignment to target
	dd = sqrt(((gx-px) * (gx-px)) + ((gy-py) * (gy-py)));
	...
	if (dd < 0.5) {		
		...
		if (dd < 0.2) {
			//moveVelocity(0.0,0.0);
			ROS_INFO("------ pathFollowRTTP : Stop dd=%.3f. ---------",dd);
			SpeedRTStop = true;
			return;
		}
	}
	...
	//ROS_INFO("----  pathFollowRTTP : ls=%.3f. as=%.3f. -------------",ls,as);
	moveVelocity(ls,as);
}

void BotNode::pathFollowRT(void) {
	ros::NodeHandle nn;
	double obsdist,an,diff,dd,as,ls,angle;
	long dleft,dright,d;	
	...
	if (quickstop) {
		quickStopMotor();
		ROS_INFO("--------- pathFollowRT : quickstop. ----------");
		return;
	}
	// check for direction alignment to target
	dd = sqrt(((gx-px) * (gx-px)) + ((gy-py) * (gy-py)));
	dd = fabs(dd);
	//ROS_INFO("------ pathFollowRT : obsdist=%.3f. dd=%.3f. ---------",obsdist,dd);
	if ((obsdist < dd) && (obsdist < 0.9)) {	
		ROS_INFO("------ pathFollowRT : obsdist=%.3f. dd=%.3f. ---------",obsdist,dd);
		obscount++;
		moveVelocity(0.0,0.0);
	...
	//ROS_INFO("----  pathFollowRT : ls=%.3f. as=%.3f. -------------",ls,as);
	moveVelocity(ls,as);
}
...
void BotNode::moveDistanceSpeed(double dist,int velocity) {
	...
	ROS_INFO(" ---- moveDistanceSpeed Dist = %.3f -------\n",dist);
	if (dist > 0.0) {
		while(true) {				
			publish_odom();
			nn.getParam("FrontObsDist",obsdist);
			//obsdist = 4.0;
			ROS_INFO(" ---- ObsDist = %.3f. dist=%.3f -------\n",obsdist,dist);
			if (obsdist > dist) {				
				break;
			}			
...
void BotNode::ptpmoveAlign()
{
	...
	angle = 57.2958 * diff;
	dd = sqrt(((gx-px) * (gx-px)) + ((gy-py) * (gy-py)));
	//ROS_INFO("------ ptpmoveAlign : gx=%.3f. gy=%.3f. px=%.3f. py=%.3f. dd=%.3f -----------",gx,gy,px,py,dd);
	moveAngle(angle);	
	usleep(200000);
	if (dd > 1.5) {
		moveDistanceSpeed(dd,1250);
	} else {
		moveDistanceSpeed(dd,900);
	}
	//sleep(1);
	//turnToOrientation();	
}
...
void BotNode::posCallback(const geometry_msgs::Twist::ConstPtr& msg)
{
	...
	if (msg->linear.z == 42.8) {  // 
		pathFollowRTSetup(msg->angular.z);
		estopped = true;
		pstate = 197;
		return;
	}
...
void BotNode::mainLoop()
{
	...
	while(ros::ok())
	{
		...
		switch (pstate) {
			...
			case 12:
        //if (quickstop) {
				if( (quickstop) || (current < -0.5) || (negativecurrent) || (ros::Time::now() > (delay_time + ros::Duration(15.0))) ) {//7
					ROS_INFO("---------- UsbEpos 12 : detected  charging current = %.3f -----------",current);
      ...
      case 122:
			...
			case 123:
				if(ros::Time::now()-delay_time > ros::Duration(1.0)) {
					...
					if ((current < -0.5) || (negativecurrent)) {
						startPMove = false;
						publish_move_status(1);
			...
			case 193:
				...
				pstate = 1930;
				break;
			case 1930:  // vision alignment
				if (adjustAGVVision()) {
					publish_move_status(1);					
				} else {
					publish_move_status(300);
					gvdy = 0.0;
				}								
				pstate = 0;
				break;
			...
			case 1951:
				...
				break;
			case 196:
				ROS_INFO("------- Epos 196 : Turn -----------");
				moveDistanceSpeedRTTurn();
				delay_time = ros::Time::now();
				SpeedRTStop = false;
				pstate = 1960;
				estopped = true;
			...
			case 197:
				...
				if (MPEndPt) {
					...
					if (MPstatus > 0.0) {
						pstate = 1971;
						//ROS_INFO("-------- pstate = 1971 ------------");
					} else {
						pstate = 1970;
						//ROS_INFO("-------- pstate = 1970 ------------");
					}
				}
				break;
			case 1970:
			...
			case 1971:
				...
				if (SpeedRTStop) {
					pstate = 197;					
					SpeedRTStop = false;
					//ROS_INFO("----------- Epos 1971 : Turning Point to Next Point  --------");
				}
				break;
			case 2:				
				if(ros::Time::now()-delay_time > ros::Duration(1.0)) {					
		...
		if (!cmd_estopped) {
			...
			// no cmd_vel from move_base so far
			if (!estopped) {
...
int main(int argc, char **argv)
{
	...
	//sleep(1);
	//botNode.moveDistance(0.4);
	//sleep(1);
	//ROS_INFO("---------------------------------- USBEpos : moveangle  Anti-Clockwise  ---------------------------");
	//botNode.moveVelocity(0.0,0.07);

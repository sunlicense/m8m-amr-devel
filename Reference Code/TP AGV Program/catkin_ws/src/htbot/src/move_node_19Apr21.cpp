...
MovNode::MovNode():
	...
	ONCEMOVEREADY(false),jackdowncmd(false),jackupcmd(false),UPJACK(false)
{
	ros::NodeHandle nh;
	...
	switch (mstate) {
					...
					if (!ONCEMOVEREADY) {
						if (MOVEREADY) {
							if ( ros::Time::now() > (clearmap_start_time + ros::Duration(5.0) )) {
								ntucjobstatus = 0;
								publish_ntuc_status(1,1,targetLP,(int)(avgenergy),ntucjobstatus);
								//publish_lift(KITCHEN,findLiftLocation(11),0,0,1);
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						...
						publish_lift(ASKLIFTTOCOME,findLiftLocation(otargetLP),0,1,0);
						mstate = 2003;						
					}
				}
				clearmap_start_time = ros::Time::now();
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						...
						publish_lift(AGVINLIFT,findLiftLocation(otargetLP),0,1,0);  // robot in lift
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						...
						//clearmap_start_time = ros::Time::now();		
						publish_sound(ASKLIFT,0,0);
						publish_lift(LIFTGOTO,1,findLiftLocation(targetLP),1,0);  // ask for lift to ground floor
						mstate = 2007;
					}
				}
				clearmap_start_time = ros::Time::now();	
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						...
						ROS_INFO("--------- MovNode 2008 : Request Lobby Door Open. -----------");
						//clearmap_start_time = ros::Time::now();	
		...
		case 2009:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {				
				if (ntucLobbyDoor == 1) {
					mstate = 2010;
					ntucDoorOpenRequestCnt = 0;
					ROS_INFO("------------- MovNode 2009 : Lobby Door Command Received. ----------------");
				} else {
					publish_fButton(301);
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (!agvjackUp) {
					if (dbRequestS("LBRA")) {
						...
						ROS_INFO("---------MovNode 2013 :  Lift Rest Area LP  Error. -----------");
					}
				}
				clearmap_start_time = ros::Time::now();	
			}
		...
		case 2054:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						// AGV turn around inside lift
						publish_lift(ASKLIFTTOCOME,findLiftLocation(otargetLP),0,1,0);  // robot in lift
						liftMechanism(false); // lower jack						
						mstate = 2055;		
					}
				}		
		...
		case 2059:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {				
				if (ntucLobbyDoor == 1) {
					mstate = 2060;
					ntucDoorOpenRequestCnt = 0;
					ROS_INFO("-------------MovNode 2059 : Lobby Door Command Received. ----------------");
				} else {
					publish_fButton(301);
					ntucDoorOpenRequestCnt++;
					if (ntucDoorOpenRequestCnt > 20) {
		...
		case 2060:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {				
				if (ntucLobbyDoor == 2) {
					...
					ntucLobbyDoor = 0;
					publish_sound(LOBBYDOOROPENED,0,0);
					ROS_INFO("------------- MovNode 2060 : Lobby Door Command Activated. ----------------");
				} else {
					ntucDoorOpenRequestCnt++;
					if (ntucDoorOpenRequestCnt > 25) {
						publish_sound(LOBBYDOORCONTROLERROR,0,0); //Lobby Door Error. Door Controller Error
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (!agvjackUp) {
					if (dbRequestS("REF")) {
						...
						ROS_INFO("---------  MovNode 2065 : REF   Error. -----------");
					}
				}
				clearmap_start_time = ros::Time::now();	
			}
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						...
						clearmap_start_time = ros::Time::now();		
						publish_sound(ASKLIFT,0,0);
						publish_lift(LIFTGOTO,1,findLiftLocation(targetLP),1,0);  
						mstate = 2161;
					}
				}
				clearmap_start_time = ros::Time::now();	
		...
		case 2162:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						publish_lift(AGVOUTSIDE,findLiftLocation(targetLP),0,0,0);
						mstate = 2163;		
					}
				}
				clearmap_start_time = ros::Time::now();	
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (!agvjackUp) {
					if (dbRequestS("LFRA")) {
						mstate = 2300; // 
						nstate = 2164;
						publish_lift(KITCHEN,findLiftLocation(targetLP),0,0,findLiftLocation(targetLP));
					} else {
		...
		case 2200: // LAUNDRYAREA_TO_LIFTAREA
			ROS_INFO("----------- MovNode : LAUNDRYAREA_TO_LIFTAREA ---------------");
		...
		case 2207:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.5)) ) {
				if (dbRequestS("LFLD")) {
					...
					mstate = 310; // 
					nstate = 2208;
					ROS_INFO("---------MovNode 2207 : Go to LFLD -----------");
					//ret = sendGoalpub(tx,ty,tz,rx,ry,rz,rw,ZSD,ZSA,NORMAL);
					//clearmap_start_time = ros::Time::now();
				} else {
					ROS_INFO("---------MovNode 2207 : Outside Lift LP Error -----------");
				} 
				clearmap_start_time = ros::Time::now();
			}
			break;
		...
		case 2210:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(3.0)) ) {
				ROS_INFO("-------------MovNode 2210 : AGV Move into lift ----------------");								
				if (dbRequestS("LFIN")) {
					movedone = 1;
					destPoint = 1;
					mstate = 310; // 
					...
					ROS_INFO("---------MovNode 2210 : Inside Lift LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						...
						publish_lift(AGVINLIFT,1,0,1,0);  // robot in lift
						liftMechanism(false); // lower jack
						ROS_INFO("---------- MovNode 2211 : Lower Jack. ----------");
						//clearmap_start_time = ros::Time::now();
						mstate = 2212;			
					}
				}	
		...
		case 2212:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (!agvjackUp) {
					// AGV rotate 180.0
					ROS_INFO("---------- MovNode 2212 : Turn around inside lift. ----------");
					movedone = 1;
					moveflag = 0;
					pos.linear.z = 42.0; // 
					pos.angular.z = 180.0;
		...
		case 2213:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						liftMechanism(true); // raise jack
						ROS_INFO("---------- MovNode 2213 : Raise Jack. ----------");
						//restoreLP();
						//clearmap_start_time = ros::Time::now();		
						//publish_sound(ASKLIFT,0,0);
						//publish_lift(LIFTGOTO,1,findLiftLocation(targetLP),1,0);  
		...
		case 2214:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (agvjackUp) {
					ROS_INFO("---------- MovNode 2214 : Ask for lift. ----------");
					restoreLP();
					//clearmap_start_time = ros::Time::now();		
					publish_sound(ASKLIFT,0,0);
					publish_lift(LIFTGOTO,1,findLiftLocation(targetLP),1,0);  
		...
		case 2215:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.0)) ) {
				if ((ntucstatus == 11) && (liftDoorOpen)) {
					...
					ntucstatus = 0;
					publish_sound(MOVEOUTOFLIFT,0,0);		
					ROS_INFO("---------- MovNode 2214 : Move out of Lift to Ward 3. ----------");	
					//restoreLP();   // restore final LP				
					movedone = 1;
					destPoint = 2;
					mstate = 310; // 		
		...
		case 2216:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						publish_lift(AGVOUTSIDE,findLiftLocation(targetLP),0,0,0);
						publish_lift(KITCHEN,findLiftLocation(targetLP),0,0,1);
						ROS_INFO("---------- MovNode 2216 : AGV outside of Lift. ----------");
						mstate = 2217;
					}
				}
				clearmap_start_time = ros::Time::now();	
		...
		case 2217:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(2.5)) ) {
				if (!agvjackUp) {
					if (dbRequestS("LFRA")) {
						//publish_lift(KITCHEN,findLiftLocation(targetLP),0,0,1);			
						ROS_INFO("--------- MovNode 2216 :   going to Lift Rest Area. -----------");			
						mstate = 2300; // 
						nstate = 2218;
					} else {

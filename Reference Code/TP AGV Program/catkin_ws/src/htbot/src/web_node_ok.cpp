...
int WEBNode::searchLP(string s)
{
	...
	rLP = -1;
	trim(lps);
	ROS_INFO("------------ WebN searchLP : lps=%s. numLP=%d -------",lps.c_str(),numLP);
	for (int i=0;i<numLP;i++) {
		s1 = LandingPointString[i];
		trim(s1);
		ROS_INFO("------------ WebN searchLP Loop : s1=%s. -------",s1.c_str());
		if (s1.compare(lps) == 0) {
			rLP = i;
			break;
		} 
...
{		
	...
	ROS_INFO("------------- WebN : readPosefromFileS ---------------");
...
bool WEBNode::webcmdServiceCallback(htbot::mqueue::Request &req,htbot::mqueue::Response &res)
{
  ...
  ROS_INFO("---------------WebNode : cmd : %d. --------------------------",cmd);
	switch (cmd) {		
		...
		case 30:
			lp = searchLP(req.lps);
			ROS_INFO("------------- WebNode 30 : lps=%s. lp=%d. ---------",req.lps.c_str(),lp);
			if (lp >=0) {
				res.tx = poseInfo[lp][0];
				res.ty = poseInfo[lp][1];
				res.tz = poseInfo[lp][2];

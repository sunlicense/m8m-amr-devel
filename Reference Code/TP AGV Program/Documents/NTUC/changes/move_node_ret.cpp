	...
	switch (mstate) {
		...
		case 1691:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (agvjackUp) {
					mstate = 1692;			
					ROS_INFO("---------- MovNode 1691 : Jack is Up. -------------");
					AGVCARRYTROLLEY = true;
				}
				clearmap_start_time = ros::Time::now();
			}
		...
		case 1692:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				...
				pos_pub.publish(pos);
				mstate = 1693;
				ROS_INFO("------------- MovNode 1692 : Rotate 90 antiClockwise. -----------");
			}
			break;
		case 1693:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
		...
		case 1694:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				...
				//if (destPoint == 2) {
					mstate = nstate;
					redirect = false;
					ROS_INFO("------- mNode 1694 : mstate = %d ------------",mstate);
				} else {
					mstate = 0;
					ROS_INFO("----------- MovNode 1694 : PickUp Completed. -----------");
				}
				ntucjobstatus = 3; // not ready
				publish_ntuc_status(1,1,targetLP,(int)(avgenergy),ntucjobstatus);
			}
		...
		case 187:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				...
				moveflag = 0;
				pos.linear.z = 42.2; // 
				trvar = findtrolleywidth((int)trolley) + 0.15;

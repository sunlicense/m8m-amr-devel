...
void WEBNode::taskCallback(const htbot::task::ConstPtr& msg)
{
	...
	switch(cmd) {
		case 2: // assigned task
			...
			if ((nfLP >= 0) && (ntLP >= 0)) {
				mqPush(nfLP);
				poseInfo[nfLP][16] = 0.0;//1.0; // activate lift trolley
				mqPush(ntLP);
				poseInfo[ntLP][16] = 0.0;//2.0; // deactivate lift
				ROS_INFO("------------- WebNode : new Task : fromLP=%d. toLP=%d. -----------",nfLP,ntLP);
			}
			break;
		case 3: // cancel task
			nfLP = (int) msg->fromLP;
...
{		
	...
	//ROS_INFO("LP Name : %s. LP : %d. py : %.3f",lps.c_str(),lp,poseInfo[lp][1]);
	return lp;
}
...
{		
  ...
  if (pfp == NULL) {
	...
	for (int j=0;j<numLP;j++) {
		...
		fprintf(pfp,"%d %s %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f\n",j,s1.c_str(),tx,ty,tz,rx,ry,rz,rw,pre_dist,pre_angle,pre_dist1,pre_angle1,post_dist,post_angle,post_dist1,post_angle1,align,func);	
	}	
  fclose(pfp);	
}
...
{		
  ...
  if (pfp == NULL) {
	...
	while(true) {
		if (fscanf(pfp,"%d %s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",&j,buf,&tx,&ty,&tz,&rx,&ry,&rz,&rw,&pre_dist,&pre_angle,&pre_dist1,&pre_angle1,&post_dist,&post_angle,&post_dist1,&post_angle1,&align,&func) == EOF) {
			break;
		}
	...
	while (true) {  	  	 	  
		...
		if (loadfire) {
			...
			if (cc1 > 25) {
				...
				ret = system("firefox -new-window http://10.0.5.102/index.html &");

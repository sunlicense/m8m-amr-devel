...
ros::Time CleanReportRT,CleanCompletedRT,SystemBackupRT;
	...
	switch (id) {
		case SYSREADY:			//1
			if (!sysready) {				
				...
				ROS_INFO("---------- TalkNode Sound : System Ready ------------");
				sleep(2);
			}
			break;
		case NEEDSPACE:		//2
			if (!needspace) {		
				...
				ROS_INFO("---------- TalkNode Sound : Need Space ------------");
				sleep(2);
			}				
			break;
		case NAVABORT: //3
			if (!navabort) {		
				...
				ROS_INFO("---------- TalkNode Sound : Nav Abort ------------");
				sleep(2);
			}
			break;
		case ESTOPREL: //4
			if (!estoprel) {			
				...
				ROS_INFO("---------- TalkNode Sound : Estop Release ------------");
				sleep(2);
			}
			break;
		case BATLOW: //5
			if (!batlow) {		
				...
				ROS_INFO("---------- TalkNode Sound : Battery Low ------------");
				sleep(2);
			}	
			break;		
		case ESTOP: //6
			if (!estop) {				
				...
				ROS_INFO("---------- TalkNode Sound : Estop On ------------");
				sleep(2);
			}
			break;
		case REACHED://7
			if (!reached) {		
				...
				ROS_INFO("---------- TalkNode Sound : Reached ------------");
				sleep(2);
			}
			break;
		case FINDPATH://8
		...
		case SHUTDOWN://13
			if (!shutdown) {	
				...
				ROS_INFO("---------- TalkNode Sound : Shutdown ------------");
				sleep(2);
			}
			break;
		case STOCKER://14
		...
		case SYSLOADED://26
			if (!sysloaded) {		
				...
				ROS_INFO("---------- TalkNode Sound : System ReadyX ------------");
				sleep(2);
			}			
			break;
		case ZEROING://27
		...
		case ASKLIFT: //53
			if (!AskLift) {		
				...
				ROS_INFO("---------- TalkNode Sound : Ask Lift ------------");
				sleep(2);
			}			
			break;
		case LIFTOPENED://54
			if (!LIftOpened) {		
				...
				ROS_INFO("---------- TalkNode Sound : Lift Opened ------------");
				sleep(2);
			}			
			break;
		case MOVEINTOLIFT://55
			if (!MoveIntoLift) {		
				...
				ROS_INFO("---------- TalkNode Sound : Move into Lift ------------");
				sleep(2);
			}			
			break;
		case MOVEOUTOFLIFT://56
			if (!MoveOutOfLift) {		
				...
				ROS_INFO("---------- TalkNode Sound : Move out of lift ------------");
				sleep(2);
			}			
			break;
		case LOBBYDOORCMDERROR://57
			if (!LobbyDoorCmdError) {		
				...
				ROS_INFO("---------- TalkNode Sound : LobbyDoorCmdError ------------");
				sleep(2);
			}			
			break;
		case LOBBYDOORCONTROLERROR://58
			if (!LobbyDoorControlError) {		
				...
				ROS_INFO("---------- TalkNode Sound : LobbyDoorControlError ------------");
				sleep(2);
			}			
			break;
		case REQUESTLOBYDOOROPEN://59
			if (!RequestLobbyDoorOpen) {		
				...
				ROS_INFO("---------- TalkNode Sound : RequestLobbyDoorOpen ------------");
				sleep(2);
			}			
			break;
		...
		case MOTORRELEASED://64
			if (!MotorReleased) {		
				...
				ROS_INFO("---------- TalkNode Sound : MotorReleased ------------");
				sleep(2);
			}			
			break;
		case MOTORENGAGED://65
			if (!MotorEngaged) {		
				...
				ROS_INFO("---------- TalkNode Sound : MotorEngaged ------------");
				sleep(2);
			}			
			break;
		case CLEANING://66
		...
		case STOPMOVESOUND:  // kill aplay 
			...
			ROS_INFO("---------- TalkNode Sound : SSMC End ------------");
			break;
	}

...
void BotNode::moveAGVintoTrolley(double trolleywidth,double trolley) {
	...
	if ((fabs(dxx) > 0.04)) {		
		...
		moveAngle(turnangle);
		usleep(300000);
		moveDistance(fabs(dxx));
		usleep(300000);
		moveAngle(-90.0);
		ROS_INFO("--------- moveAGVintoTrolley : Adjust. Turn=%3f. Move=%.3f -----------",turnangle,fabs(dxx)); 
	} else {
		if (fabs(dan) > 0.05) {
			...
			ROS_INFO("--------- moveAGVintoTrolley : No Adjustment needed. dxx=%.3f. dan=%.3f -----------",dxx,dan);
		}
	}

...
void laserCallBack(const sensor_msgs::LaserScan::ConstPtr& lscan) {	
	...
	if (!slaserStarted) {
		...
		if (fp == NULL) {
  		ROS_INFO("I couldn't open Left Laserfile for writing.\n");    
  		return;
  	}
		slaserStarted = true;
		laser_size = lscan->ranges.size();
		maxid = laser_size;
		ROS_INFO("****************  Left Laser Size : %d  ***********************",laser_size);
		//fprintf(fp,"%d\n",MAX_IDX);
		fprintf(fp,"Left Laser Size = %d\n",laser_size);
		for (int i=0;i<laser_size;i++) {
			// calcuate range based on geometry
			an = ((PI_2-THET_R)+DEG2RAD(i));
			if (an < (PI_2-THET_R1)) {
				range = LTT/cos(an);
			} else {
				if (an < PI_2) {
		...
		ROS_INFO(" ************* Left Laser Template Profile Generated *****************");
		fclose(fp);
		return;
	}
	if (maxid == 0) {
	...
	*/
	...
	nx.getParam("CentralLaserObsDist",CentralLaserObsDist);	
	if (CentralLaserObsDist < 2.0) {
		//ROS_INFO("****** Left Laser CentralLaserObsDist = %.3f **********",CentralLaserObsDist);
		scan_msg.ranges[0] = CentralLaserObsDist ;
	}

	scan_pub.publish(scan_msg);
	nx.setParam("LeftLaserStatus",0);	

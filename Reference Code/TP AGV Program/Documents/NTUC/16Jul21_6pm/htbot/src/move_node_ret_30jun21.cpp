			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						...
						mstate = 181;
						clearmap_start_time = ros::Time::now();
						ROS_INFO("----- mNoveret 180 : Start of dropping trolley. Move straight --------------");
					}
				}
			}
			break;
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						...
						pos_pub.publish(pos);
						mstate = 182;
						ROS_INFO("----- mNoveret 181 : Start of dropping trolley. Turn 90 --------------");
					}
				}
			}
			break;
				...
				if (movedone == 0) {
					if (moveflag == 1) {
						...
						if (redirect) {
							mstate = nstate;
							redirect = false;
							ROS_INFO("-------------- MovNode 186 : mstate = %d ------------------",mstate);
						} else {
							mstate = 0;
						...
						if (low_power) {
							...
							ROS_INFO("------- mNodeRet 186 : Job Done. ----------");
						}
					}
				}
				clearmap_start_time = ros::Time::now();

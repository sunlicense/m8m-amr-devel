...
MovNode::MovNode():
	...
	cleanplanmove(false),racplanMove(false),racplanMoveObs(false),NTUCNav(true),
	ntucTest(false),ntucstatus(0),ntucLobbyDoor(0),NTUCINTERNALTEST(false),liftDoorOpen(false),ntucjobstatus(3),
	ntucDistToLobbyDoorOutside(1.5),ntucDistToLobbyDoorInside(1.5),agvjackUp(false),LoadingBayStatus(false)
{
	ros::NodeHandle nh;
  ...
  // Uno
	...
	ntucstatus_pub = nh.advertise<htbot::ntuc_status>("to_fleet", 100);
...
int MovNode::findfloorLP(int LP) 
{
	int currentfloor;
...
void MovNode::publish_ntuc_status(int cmd, int status, int location, int b_level, int complete)
{
	htbot::ntuc_status msg;
	msg.cmd = cmd;
	msg.status = status;
...
void MovNode::restoreLP(void) {
	...
	post_dist1 = post_distm;
	post_angle1 = post_anglem;
	targetLP = targetLPm;
	cLPName = cLPNamem;
	align = alignm;
	func = funcm;
...
bool MovNode::dbRequestSP(std::string lps) {
	...
	if (web_client.call(web_srv)) {
		if (web_srv.response.status == 30) {	 
			...
			pre_dist1m = pre_dist;
			pre_angle1m = pre_angle;
			post_dist1m = post_dist;
			post_angle1m = post_angle;
			targetLPm = targetLP;
			cLPNamem = cLPName;
			alignm = align;
			funcm = func;
...
bool MovNode::planning(void) {
	...
	if ((otargetLP > 1) && (otargetLP <= 20) ) {
		// LIFTAREA
		if ((targetLP >= 50) && (targetLP <= 60) ) {
			// LIFTAREA to LOBBYAREA
			mstate = LIFTAREA_TO_LOBBYAREA;
		} else {
			if ((targetLP >= 100) && (targetLP <= 120) ) {
		...
		if ((otargetLP >= 50) && (otargetLP <= 60) ) {
			// LOBBYAREA
			if ((targetLP >= 2) && (targetLP <= 20) ) {
			...
			if ((otargetLP >= 100) && (otargetLP <= 120) || ((otargetLP >= 0) && (otargetLP < 2)) ) {
				// LAUNDRYAREA
				if ((targetLP >= 2) && (targetLP <= 20) ) {
	...
	switch (mstate) {
				...
				if (movedone == 0) {
					if (moveflag == 1) {
						if (dbRequestSP("OUTSIDELIFTLP")) {
							...
							ROS_INFO("--------- Outside Lift LP Error -----------");
						}
					} 
				} 
			}
		...
		case 2002: // Call for Lift.
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				...
				publish_lift(ASKLIFTTOCOME,findfloorLP(otargetLP),0,1);
				mstate = 2003;
			}
			break;
		case 2003:
		...
		case 2004:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				...
				publish_lift(AGVINLIFT,findfloorLP(otargetLP),0,1);  // robot in lift
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						...
						clearmap_start_time = ros::Time::now();		
						publish_sound(ASKLIFT,0,0);
						publish_lift(LIFTGOTO,1,findfloorLP(otargetLP),1);  // ask for lift to ground floor
						mstate = 2007;
					}
				}
				clearmap_start_time = ros::Time::now();	
				...
				if (agvjackUp) {
					if ((ntucstatus == 10) && (liftDoorOpen)) {
						...
						if (dbRequestS("INSIDELOBBYLP")) {
							...
							ROS_INFO("--------- Inside Lobby LP  Error. -----------");
						}
					}
				}
				clearmap_start_time = ros::Time::now();	
		...
		case 2013:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				if (dbRequestS("LOBBYRESTAREALP")) {
					mstate = 2300; // 
					nstate = 2014;
				} else {
					ROS_INFO("---------  Lift Rest Area LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		case 2054:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				// AGV turn around inside lift
				publish_lift(ASKLIFTTOCOME,findfloorLP(otargetLP),0,1);  // robot in lift
				liftMechanism(false); // lower jack
				clearmap_start_time = ros::Time::now();
				mstate = 2055;				
			}
		...
		case 2061:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.0)) ) {
				if (dbRequestS("LOADINGBAYLOBBYLP")) {
					movedone = 1;
					destPoint = 2;
					mstate = 310; // 
					nstate = 2062;
					...
					ROS_INFO("--------- LoadingBay_lobby LP  Error. -----------");
				}			
			}
			break;
		case 2062: 
		...
		case 2101:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						if (dbRequestSP("LOADINGBAYLOBBYLP")) {
							movedone = 1;
							destPoint = 2;
							mstate = 310; // 
							nstate = 2102;
				...
				if (movedone == 0) {
					if (moveflag == 1) {
						if (dbRequestSP("OUTSIDELOBBYLP")) {
							...
							ROS_INFO("--------- Outside_lobby LP  Error -----------");
						}
					} 
				} 
			}
		...
		case 2156:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {
				publish_sound(ASKLIFT,0,0);
				ROS_INFO("------------- AGV Requesting for Lift  ----------------");
				publish_lift(ASKLIFTTOCOME,1,0,1);
				mstate = 2157;
			}
			break;
		case 2157:
		...
		case 2158:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.5)) ) {
				// AGV turn around inside lift
				publish_lift(AGVINLIFT,1,0,1);  // robot in lift
				liftMechanism(false); // lower jack
				clearmap_start_time = ros::Time::now();
				mstate = 2159;				
			}
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.3)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						...
						clearmap_start_time = ros::Time::now();		
						publish_sound(ASKLIFT,0,0);
						publish_lift(LIFTGOTO,1,findfloorLP(targetLP),1);  
						mstate = 2161;
					}
				}
				clearmap_start_time = ros::Time::now();	
		...
		case 2162:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						publish_lift(AGVOUTSIDE,findfloorLP(targetLP),0,0);
						if (func == 2.0) {
							mstate = 2163;
						} else {
							mstate = 0;
		...
		case 2201:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						if (dbRequestSP("LOADINGBAYLAUNDRYLP")) {
							movedone = 1;
							destPoint = 2;
							mstate = 310; // 
							nstate = 2202;
							...
							ROS_INFO("--------- LoadingBay_laundry LP  Error -----------");
						}
					} 
				} 
			}
		...
		case 2203:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (dbRequestS("OUTSIDELOBBYLP")) {
					...
					ROS_INFO("--------- Outside Lobby LP  Error -----------");
				} 
			}
			break;
		case 2204:

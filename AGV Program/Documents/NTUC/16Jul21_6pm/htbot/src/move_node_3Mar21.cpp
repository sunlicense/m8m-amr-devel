...
public:
	...
	double delay;
	double align,function;  // function = 0.0 > no lifting of trolley. 1.0 > lifting of trolley. 2.0 > check loading bay
	int numLPInQ;
	int readynav;
	...
	// ntuc
	bool ntucTest,NTUCINTERNALTEST,liftDoorOpen,agvjackUp,loadingbayempty;
...
MovNode::MovNode():
	...
	cleanplanmove(false),racplanMove(false),racplanMoveObs(false),
	ntucTest(false),ntucstatus(0),ntucLobbyDoor(0),NTUCINTERNALTEST(false),liftDoorOpen(false),ntucjobstatus(3),
	ntucDistToLobbyDoorOutside(1.5),ntucDistToLobbyDoorInside(1.5),agvjackUp(false),loadingbayempty(false)
{
	ros::NodeHandle nh;
...
{
	if (updown) {
		publish_fButton(601);  // instruct arduino to up lift mechanism
	} else {
		publish_fButton(604);  // instruct arduino to down lift mechanism
	}
}
...
bool MovNode::mqPop() {
	...
	if (mq_client.call(mq_srv)) {
		if (mq_srv.response.status == 1) {	 
			...
			function = mq_srv.response.function;
...
bool MovNode::dbRequest(int cmd, int GP, int LP) {
	...
	if (web_client.call(web_srv)) {
		...
		if (web_srv.response.status == 32) {	 
			...
			function = web_srv.response.function;
			ret = true;
		} else {

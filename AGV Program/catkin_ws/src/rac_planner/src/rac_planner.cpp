...
void RacPlanner::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros){
    ...
    world_model_ = new base_local_planner::CostmapModel(*costmap_);
		...
		nh.getParam("racpathDir",racpathDir);
		nh.getParam("racpath",racpath);
		//loadPath();
		cleanplanmove = false;
		PPPtr = 0;
		initialized_ = true;
...
void RacPlanner::loadPath(void)
{
	...
	nh.getParam("normalPath",normalPath);
	if (normalPath) {
		fn = racpathDir + racpath;
		ROS_INFO("----- RAC Planner : Normal : makePlan Fn = %s. --------",fn.c_str());
		fp = fopen(fn.c_str(), "r");
		if (fp == NULL) {
  		ROS_INFO("---- RACPlanner loadPath : I couldn't open normalpath file for reading. ----------");    
  		return;
 	 	}
		noPP = 0;
		while(true) {
			...
			pathInfo[noPP][5] = prz;
			pathInfo[noPP][6] = prw;
			pathInfo[noPP][7] = 0; // 0.0=normal. 1.0=segment marker. segment end point
			noPP++;
		}
	} else {
		fn = cleanplanDirectory + cleanplan;
		ROS_INFO("----- RAC Planner : Cleaning : makePlan Fn = %s. --------",fn.c_str());
		fp = fopen(fn.c_str(), "r");
		if (fp == NULL) {
  		ROS_INFO("---- RACPlanner loadPath : I couldn't open cleanplan file for reading. ----------");    
  		return;
 	 	}
...
void RacPlanner::loadNormalPath(void)
{
	ros::NodeHandle nh;
	//bool normalPath;
	FILE *fp;
	...
	if (normalPath) {
		fn = racpathDir + racpath;
		ROS_INFO("################ RAC Planner : loadNormalPath : makePlan Fn = %s. ##################",fn.c_str());
		fp = fopen(fn.c_str(), "r");
		if (fp == NULL) {
  		ROS_INFO("---- RACPlanner loadNoarmPath : I couldn't open normalpath file for reading. ----------");    
  		return;
 	 	}
		//ROS_INFO("-------------------------- RACPLANNER : LoadNormalPath 1 ----------------------------");
		noPP = 0;
		while(true) {
			if (fscanf(fp,"%lf %lf %lf %lf %lf %lf %lf %lf\n",&px,&py,&pz,&prx,&pry,&prz,&prw,&status) == EOF) {
				break;
			...
			pathInfo[noPP][7] = 0; // 0.0=normal. 1.0=segment marker. segment end point
			noPP++;
			//ROS_INFO("--------- RACPLANNER : LoadNormalPath np=%d. -----------------------",noPP);
		}
	} 
	fclose(fp);
	//ROS_INFO("-------------------------- RACPLANNER : LoadNormalPath exit ----------------------------");
}
...
bool RacPlanner::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal,  std::vector<geometry_msgs::PoseStamped>& plan ){
  ...
  double wy = goal.pose.position.y;
	...
	//ROS_INFO("------------------- RACPlanner : Here 2 ----------------------");
	if (normalPath) {
	//if (cleanplanmove) {
		
		plan.push_back(start);
		ros::Time plan_time = ros::Time::now();
		//ROS_INFO("------------------- RACPlanner : Here 3 ----------------------");
		for (i=0;i<(noPP-1);i++) {		
			//status = pathInfo[i][7];
			//if (status == 1.0) {
		...
		ROS_INFO("----------- rac planner : makePlan Done. noPP=%d -------------",noPP);
	} else {
		return false;
	}
  return true;

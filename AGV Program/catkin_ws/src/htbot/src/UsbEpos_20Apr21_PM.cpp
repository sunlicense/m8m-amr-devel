...
void BotNode::moveAGVintoTrolley(double trolleywidth,double trolley) {
	...
	double BOXW,BOXL;
	double vl,vr,vd,vb;
	bool RightLeg,LeftLeg,RightZone,LeftZone;
	...
	n.setParam("STARTDETECTLEG",true); // initiate left and right laser detection of the legs
	usleep(200000);
	cnt = 0;
	while (true) {
		cnt++;
		...
		ROS_INFO("--- Trolley : rlidx=%d. rldist=%.3f. rlAn=%.3f. llidx=%d. lldist=%.3f. llAn=%.3f. ---",RightLegIdx,\
									RightLegdist,RightLegAngle,LeftLegIdx,LeftLegdist,LeftLegAngle);
		if (cnt > 10) {
			ROS_INFO("------- Trolley Alignment ------------------------");
			break;
		}
		usleep(500000);
	}

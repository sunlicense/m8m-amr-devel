...
*		Changes :
			15.6.21 : 7.05pm
				1. added taks 5 & 6 to pick and drop trolley at LP
			12.7.21 : 11am
				2. added testing vision detection
			15.7.21 : 4.40pm
				1. remove launching of firefox
*/
	...
	switch(cmd) {
		case 2: // assigned task
			...
			if ((nfLP >= 0) && (ntLP >= 0)) {
				...
				mqPush(ntLP);
				poseInfo[ntLP][16] = 2.0; //2.0; // dropoff LP
				poseInfo[ntLP][17] = (double)type;
				switch(type) {
					case 1: // LxB
				...
				ROS_INFO("------------- WebNode : new Task : fromLP=%d. toLP=%d. trolley=%d. -----------",nfLP,ntLP,type);
			}
			break;
		case 3: // cancel task
			nfLP = (int) msg->fromLP;
		...
		case 4: // assigned special one LP task
			...
			if ((ntLP >= 0)) {
				if (ntLP >= 73) {
					mqPush(ntLP);
					poseInfo[ntLP][16] = 0.0; //do nothing
					poseInfo[ntLP][17] = (double)type;
				} else {
					mqPush(ntLP+LPSHIFT);
				...
				ROS_INFO("------------- WebNode : new Special One LP Task : toLP=%d. trolley=%d-----------",ntLP,type);
			}
			break;
		case 5: // assigned special one LP task to pick
			ntLP = (int) msg->toLP;
			type = (int) msg->type;
			rn.setParam("NTUCNav",true);
			if ((ntLP >= 0)) {
				if (ntLP >= 73) {
					mqPush(ntLP);
					poseInfo[ntLP][16] = 1.0; //do pick
					poseInfo[ntLP][17] = (double)type;
				} else {
					mqPush(ntLP);
					poseInfo[ntLP][16] = 1.0; //do nothing
					poseInfo[ntLP][17] = (double)type;
				}
				ROS_INFO("------------- WebNode : new Special One LP Task : toLP=%d. trolley=%d-----------",ntLP,type);
			}
			break;
		case 6: // assigned special one LP task to drop
			ntLP = (int) msg->toLP;
			type = (int) msg->type;
			rn.setParam("NTUCNav",true);
			if ((ntLP >= 0)) {
				mqPush(ntLP+LPSHIFT);
				poseInfo[ntLP+LPSHIFT][16] = 2.0; //do drop
				poseInfo[ntLP+LPSHIFT][17] = (double)type;
				ROS_INFO("------------- WebNode : Pick Task : toLP=%d. trolley=%d-----------",ntLP+LPSHIFT,type);
			}
			break;
		case 110:
			type = (int) msg->type;
			if (type == 1) {
				rn.setParam("ONCEMOVEREADY",true);
			}
			break;
	}
}
  ...
  //ROS_INFO("---------------WebNode : cmd : %d. --------------------------",cmd);
	switch (cmd) {		
		...
		case 105:
			...
			break;
		case 106:
			ROS_INFO("---------- WebNode : 106 publish sound SSMC----------------");
			//publish_sound(SSMC,0,0);
			testsound();
			break;
		case 107:
			ROS_INFO("---------- WebNode : 107 publish sound END SSMC----------------");
			publish_sound(STOPMOVESOUND,0,0);
			break;
		case 108:
			ROS_INFO("------ WebNode : Trolley Front -------------");
			pos.linear.z = 42.3; //
			pos.linear.x = 1.0; //  trolley type
			pos.angular.z = 0.33; // trolley width
			pos_pub.publish(pos);
		...
		case 109:
			...
			break;
		case 1090:
			ROS_INFO("------ WebNode : Vision Adjust -------------");
			pos.linear.z = 42.5; //
			pos.linear.x = 1.0; //  trolley type
			pos.angular.z = 0.33; // trolley width
			pos_pub.publish(pos);

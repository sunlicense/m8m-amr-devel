...
BotNode::BotNode():
  ...
  alignflag(false),multiLP(false),maxautolinear(0.5),maxautoangular(0.5),maxjoyangular_(0.6),maxjoylinear_(0.6),
	pauseRobotMove(false),manualvelflag(0)
{
...
void BotNode::moveAGVintoTrolley(double trolleywidth,double trolley) {
	...
	agvx = AGVLENHALF * sin(fabs(dan));
	if (dxx > 0.0) {		
		// AGV on the right side of center
		turnangle = 90.0 - (dan * 57.2958);  // turn left				
	} else {
		// AGV on the left side of center
		turnangle = -90.0 - (dan * 57.2958);  // turn right	
	}
	if (fabs(turnangle) > 90.0) {
		dxxx = dxx - agvx;
	} else {
		dxxx = dxx + agvx;
	}
	ROS_INFO("--------- moveAGVintoTrolley : Adjust. Turn=%3f. Move=%.3f. dan=%.3f -----------",turnangle,fabs(dxxx),dan); 
	return;
	if ((fabs(dxxx) > 0.04)) {		
		moveAngle(turnangle);
		usleep(300000);
	...
	n.setParam("RightZone",RightZone);
	n.setParam("LeftZone",LeftZone);
	n.setParam("STARTDETECTLEG",true);
	vb = 0.085;
	vl = vb;
	vr = vb;
	vd = 0.025;
	sleep(1);
	ROS_INFO("------- moveAGVintoTrolley : Start Moving into Trolley Alignment -----------");
	
	while(true) {					
		if (!RightLeg) {
			n.getParam("RightLeg",RightLeg);
			if (RightLeg) {
				vr = 0.0;
			} else {
				n.getParam("RightZone",RightZone);
				if (RightZone) {
					vr = vb + vd;
				} else {
					vr = vb;
		...
		if (!LeftLeg) {
			n.getParam("LeftLeg",LeftLeg);
			if (LeftLeg) {
				vl = 0.0;
			} else {
				n.getParam("LeftZone",LeftZone);
				if (LeftZone) {
					vl = vb + vd;
				} else {
					vl = vb;
		...
		//ROS_INFO("----- MoveintoTrolley : vl=%.3f. vr=%.3f. --------",vl,vr);
		usleep(10000);		
		publish_odom();	
	}
	n.setParam("STARTDETECTLEG",false);
	usleep(500000);
	ROS_INFO("------------ UsbEpos : Move Straight for trolley width ------------------");
	moveDistance(trolleywidth);	
	usleep(50000);
	//moveAngle(90);
	...
	while(ros::ok())
	{
      ...
      case 122:
			...
			case 1230:
				if(ros::Time::now()-delay_time > ros::Duration(0.3)) {
					if ((current < 0.0) || (negativecurrent)) {
						...
						estopped = false;
						negativecurrent = false;
						//ROS_INFO(" ============== Negative BtMode current = %.3f =============",current);
					} else {
						curcnt++;
						if (curcnt > 3) {
							pstate = 124; //0							
			...
			case 160:
				...
				break;
			case 17:  // special move into trolley
				moveAGVintoTrolley(trolleywidth,trolley);
				pstate = 170;
				break;
			case 170:
				if(ros::Time::now()-delay_time > ros::Duration(0.2)) {
		...
		if (!cmd_estopped) {
			...
			if (!estopped) {	
				if (checkDist) {	
					...
					*/					
					...
					if ((tdist < slowDist) ) {	
						...
						if ((linear_ == 0.0) && (angular_ == 0.0) && (tdist < 0.25) && !reach && (navstatus == 7) ) {
					...
					/*
					if ( linear_ < 0.0 ) {
						ROS_INFO("-------- USBEpos : Negative Velocity vel = %.3f --------------",linear_);
						if ( (linear_ < 0.0) ) {
						//if ( (obsdist < 0.35) || (mapobsdist < 0.35) ) {
						...
						//if ( (obsdist < 0.3) || (mapobsdist < 0.3) || (leftlaserobsdist < 0.4) || (rightlaserobsdist < 0.4) ) {
							...
							if ((!escape)) {	
								escape_cnt++;		
								ROS_INFO("------------- USBEpos : Negative Velocity cnt = %d.  -----------------",escape_cnt);
								if (escape_cnt >= 6) {
									//escapeTime = ros::Time::now(); //good
									//escape = true;                 //good
									//escapeNext = false;            //good
									//findpathflag = false;          //good
									//publish_sound(STOPMOVESOUND,0,0); //good
									cancel_goal();
				...
				if (!estop_pressed) { 
					...
					//ROS_INFO("---------- BotNode : linear_ = %.2f. angular_ = %.2f. --------------",linear_,angular_);
					moveVelocity(linear_,angular_);		
				}
			...
			if (!estopped) {
				...
				//if (ros::Time::now()- last_joycmd_time < ros::Duration(1.0)) {
				//	moveVelocity(joylinear_,joyangular_);
				//} else {
				//	joylinear_ = joyangular_ = 0.0;
				//	moveVelocity(joylinear_,joyangular_);
				//}
			}
		}

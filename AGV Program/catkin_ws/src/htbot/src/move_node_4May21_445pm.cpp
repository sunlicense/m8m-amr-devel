...
void MovNode::moveLPLoop()
{
	...
	switch (mstate) {
		...
		case 2156:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (dbRequestS("LFLD")) {
					...
					ROS_INFO("---------MovNode 2156 : Outside Lift LP Error -----------");
				} 
				clearmap_start_time = ros::Time::now();
			}
			break;
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						...
						mstate = 2172;		// reached liftarea ward
					}
				}
				clearmap_start_time = ros::Time::now();	
			}
		...
		case 2363:
			...
			break;
		case 2400:  // return to dock. LIFTAREA_TO_DOCK
			ROS_INFO("----------- MovNode 2400 : RETURN TO DOCK. LIFTAREA_TO_DOCK ---------------");
			mstate = 2300; // pre-move
			nstate = 2401;
			break;
		case 2401:
				...
				if (movedone == 0) {
					if (moveflag == 1) {
						if (dbRequestS("LFLD")) {
							...
							ROS_INFO("--------- MovNode 2401 : LFLD Error -----------");
						}
					} 
				} 
				clearmap_start_time = ros::Time::now();
		...
		case 2402:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if (movedone == 0) {
					if (moveflag == 1) {
						publish_sound(ASKLIFT,0,0);
						ROS_INFO("-------------MovNode 2402 : AGV Requesting for Lift  ----------------");
						publish_lift(ASKLIFTTOCOME,currentfloor,0,1,0);
						mstate = 2403;
					}
				}
		...
		case 2403:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				if ((ntucstatus == 10) && (liftDoorOpen)) {
					...
					liftDoorOpen = false;
					mstate = 2404;
					ROS_INFO("------------- MovNode 2403 : AGV Move into lift ----------------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
		...
		case 2405:
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {
				...
				if (dbRequestS("LFIN")) {
					...
					ROS_INFO("---------MovNode 2405 : Inside Lift LP  Error. -----------");
				}
				clearmap_start_time = ros::Time::now();	
			}
			break;
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(0.2)) ) {	
				if (movedone == 0) {
					if (moveflag == 1) {		
						clearmap_start_time = ros::Time::now();		
						publish_sound(ASKLIFT,0,0);
						publish_lift(LIFTGOTO,1,1,1,0);  // ask for lift to ground floor
						mstate = 2409;
					}
				}
				clearmap_start_time = ros::Time::now();	
			...
			if ( ros::Time::now() > (clearmap_start_time + ros::Duration(1.0)) ) {
				...
				if (ProfileMoveObsDist > DistClearance) {					
					...
					if (dbRequestS("LFLBD")) {
						...
						ROS_INFO("--------- MovNode 2410 : LFLBD  Error. -----------");
					}
				}
				clearmap_start_time = ros::Time::now();	
			}
